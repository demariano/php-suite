...
export class TownDatabaseService implements TownDatabaseServiceAbstract {
    ...
    async findRecordContainingName(name: string): Promise<TownDto[] | null> {
        const townRecords = await this.townTable.find(
            {
                GSI1PK: 'TOWN',
            },
            {
                where: 'contains(${townName}, @{townName})',
                substitutions: {
                    townName: name,
                },
                index: 'GSI1',
    ...
    async findRecordByStatusAndAreaId(status: string, areaId: string): Promise<TownDto[] | null> {
        const townRecords = await this.townTable.find(
            {
