import { Entity } from 'dynamodb-onetable';

export const InvoicingSchema = {
    version: '0.0.1',
    indexes: {
        primary: { hash: 'PK', sort: 'SK' },
        GSI1: { hash: 'GSI1PK', sort: 'GSI1SK' },
        GSI2: { hash: 'GSI2PK', sort: 'GSI2SK' },
        GSI3: { hash: 'GSI3PK', sort: 'GSI3SK' },
        GSI4: { hash: 'GSI4PK', sort: 'GSI4SK' },
        GSI5: { hash: 'GSI5PK', sort: 'GSI5SK' },
        GSI6: { hash: 'GSI6PK', sort: 'GSI6SK' },
        GSI7: { hash: 'GSI7PK', sort: 'GSI7SK' },
        GSI8: { hash: 'GSI8PK', sort: 'GSI8SK' },
        GSI9: { hash: 'GSI9PK', sort: 'GSI9SK' },
        GSI10: { hash: 'GSI10PK', sort: 'GSI10SK' },
        GSI11: { hash: 'GSI11PK', sort: 'GSI11SK' },
        GSI12: { hash: 'GSI12PK', sort: 'GSI12SK' },
        GSI13: { hash: 'GSI13PK', sort: 'GSI13SK' },
    },
    models: {
        Invoice: {
            PK: { type: String, value: 'INVOICE', hidden: false },
            SK: { type: String, value: '${invoiceId}', hidden: false },
            invoiceId: { type: String, generate: 'ulid' },
            docno: { type: String },
            invoiceDate: { type: String },
            changeReason: { type: String, required: false },
            customerId: { type: String },
            customerName: { type: String },
            areaId: { type: String },
            areaName: { type: String },
            territoryManagerId: { type: String },
            territoryManagerName: { type: String },
            salesTypeId: { type: String },
            salesTypeName: { type: String },
            finalAmount: { type: Number },
            invoiceAmount: { type: Number },
            taxAmount: { type: Number },
            totalAmountPaid: { type: Number, required: false, default: 0 },
            contractId: { type: String },
            contractName: { type: String },
            termsId: { type: String },
            termsName: { type: String },
            productPriceTypeId: { type: String },
            productPriceTypeName: { type: String },
            contractSales: { type: Boolean, required: false },
            status: {
                type: String,
                enum: ['ACTIVE', 'INACTIVE', 'FOR_APPROVAL', 'FOR_DELETION', 'FOR_DEACTIVATION', 'NEW_RECORD', 'DRAFT'],
                required: false,
            },
            printStatus: { type: String, enum: ['PENDING', 'COMPLETED', 'FOR_REPRINT'], required: false },
            paymentStatus: { type: String, enum: ['PENDING', 'PARTIAL', 'PAID', 'OVERPAID'], required: false },
            activityLogs: { type: Array },
            forApprovalVersion: { type: Object },
            approverMessage: { type: String, required: false },
            invoiceDetails: { type: Array },
            payments: { type: Array, required: false, default: [] },
            GSI1PK: { type: String, value: 'INVOICE', hidden: false },
            GSI1SK: { type: String, value: '${invoiceId}', hidden: false },
            GSI2PK: { type: String, value: 'INVOICE#${status}', hidden: false },
            GSI2SK: { type: String, value: '${docno}', hidden: false },
            GSI3PK: { type: String, value: 'INVOICE#${customerId}', hidden: false },
            GSI3SK: { type: String, value: '${invoiceDate}', hidden: false },
            GSI4PK: { type: String, value: 'INVOICE#${salesTypeId}', hidden: false },
            GSI4SK: { type: String, value: '${invoiceDate}', hidden: false },
            GSI5PK: { type: String, value: 'INVOICE#${contractId}', hidden: false },
            GSI5SK: { type: String, value: '${invoiceDate}', hidden: false },
            GSI6PK: { type: String, value: 'INVOICE#${termsId}', hidden: false },
            GSI6SK: { type: String, value: '${invoiceDate}', hidden: false },
            GSI7PK: { type: String, value: 'INVOICE#${productPriceTypeId}', hidden: false },
            GSI7SK: { type: String, value: '${invoiceDate}', hidden: false },
            GSI8PK: { type: String, value: 'INVOICE#${areaId}', hidden: false },
            GSI8SK: { type: String, value: '${invoiceDate}', hidden: false },
            GSI9PK: { type: String, value: 'INVOICE#${territoryManagerId}', hidden: false },
            GSI9SK: { type: String, value: '${invoiceDate}', hidden: false },
            GSI10PK: { type: String, value: 'INVOICE#${paymentStatus}', hidden: false },
            GSI10SK: { type: String, value: '${invoiceDate}', hidden: false },
            GSI11PK: { type: String, value: 'INVOICE', hidden: false },
            GSI11SK: { type: String, value: '${invoiceDate}', hidden: false },
            GSI12PK: { type: String, value: 'INVOICE', hidden: false },
            GSI12SK: { type: String, value: '${docno}', hidden: false },
            GSI13PK: { type: String, value: 'INVOICE#${customerId}#${paymentStatus}#${status}', hidden: false },
            GSI13SK: { type: String, value: '${docno}', hidden: false },
        },
        SalesType: {
            PK: { type: String, value: 'SALES_TYPE', hidden: false },
            SK: { type: String, value: '${salesTypeId}', hidden: false },
            status: {
                type: String,
                enum: ['ACTIVE', 'INACTIVE', 'FOR_APPROVAL', 'FOR_DELETION', 'FOR_DEACTIVATION', 'NEW_RECORD', 'DRAFT'],
                required: false,
            },
            salesTypeId: { type: String, generate: 'ulid' },
            salesTypeName: { type: String },
            allowDiscount: { type: Boolean, required: false },
            contractSales: { type: Boolean, required: false },
            defaultDiscount: { type: Number, required: false, default: 0 },
            defaultTax: { type: Number, required: false, default: 0 },
            incomeGenerating: { type: Boolean, required: false },
            taxable: { type: Boolean, required: false },
            activityLogs: { type: Array },
            approverMessage: { type: String, required: false },
            forApprovalVersion: { type: Object },
            changeReason: { type: String, required: false },
            GSI1PK: { type: String, value: 'SALES_TYPE', hidden: false },
            GSI1SK: { type: String, value: '${salesTypeName}', hidden: false },
            GSI2PK: { type: String, value: 'SALES_TYPE#${status}', hidden: false },
            GSI2SK: { type: String, value: '${salesTypeName}', hidden: false },
        },
        TerritoryManager: {
            PK: { type: String, value: 'TERRITORY_MANAGER', hidden: false },
            SK: { type: String, value: '${territoryManagerId}', hidden: false },
            territoryManagerId: { type: String, generate: 'ulid' },
            territoryManagerName: { type: String },
            activityLogs: { type: Array },
            approverMessage: { type: String, required: false },
            forApprovalVersion: { type: Object },
            contactNo: { type: String, required: false },
            changeReason: { type: String, required: false },
            status: {
                type: String,
                enum: ['ACTIVE', 'INACTIVE', 'FOR_APPROVAL', 'FOR_DELETION', 'FOR_DEACTIVATION', 'NEW_RECORD', 'DRAFT'],
                required: false,
            },
            GSI1PK: { type: String, value: 'TERRITORY_MANAGER', hidden: false },
            GSI1SK: { type: String, value: '${territoryManagerName}', hidden: false },
            GSI2PK: { type: String, value: 'TERRITORY_MANAGER#${status}', hidden: false },
            GSI2SK: { type: String, value: '${territoryManagerName}', hidden: false },
        },
        Contract: {
            PK: { type: String, value: 'CONTRACT', hidden: false },
            SK: { type: String, value: '${contractId}', hidden: false },
            contractId: { type: String, generate: 'ulid' },
            contractNo: { type: String },
            contractName: { type: String },
            customerId: { type: String },
            customerName: { type: String },
            areaId: { type: String },
            areaName: { type: String },
            startDate: { type: String },
            endDate: { type: String },
            contractType: { type: String, enum: ['REGULAR', 'CONTRACT_PER_INVOICE'], required: false },
            contractAmount: { type: Number },
            totalAmountPaid: { type: Number, required: false, default: 0 },
            contractProductDeals: { type: Array },
            payments: { type: Array, required: false, default: [] },
            invoicedAmount: { type: Number },
            deliveryStatus: { type: String, enum: ['PENDING', 'DELIVERED'], required: false },
            paymentStatus: { type: String, enum: ['PENDING', 'PARTIAL', 'PAID', 'OVERPAID'], required: false },
            deliveredAmount: { type: Number },
            changeReason: { type: String, required: false },
            activityLogs: { type: Array },
            forApprovalVersion: { type: Object },
            approverMessage: { type: String, required: false },
            rebatePercentage: { type: Number, required: false },
            rebateType: { type: String, enum: ['PERCENTAGE', 'AMOUNT', 'NONE'], required: false },
            rebateAmount: { type: Number, required: false },
            rebateClaimedAmount: { type: Number, required: false },
            rebateClaimedStatus: { type: String, enum: ['PENDING', 'CLAIMED', 'NOT_CLAIMED'], required: false },
            status: {
                type: String,
                enum: ['ACTIVE', 'INACTIVE', 'FOR_APPROVAL', 'FOR_DELETION', 'FOR_DEACTIVATION', 'NEW_RECORD', 'DRAFT'],
                required: false,
            },
            GSI1PK: { type: String, value: 'CONTRACT', hidden: false },
            GSI1SK: { type: String, value: '${contractNo}', hidden: false },
            GSI2PK: { type: String, value: 'CONTRACT#${status}', hidden: false },
            GSI2SK: { type: String, value: '${contractNo}', hidden: false },
            GSI3PK: { type: String, value: 'CONTRACT#${customerId}', hidden: false },
            GSI3SK: { type: String, value: '${contractNo}', hidden: false },
            GSI4PK: { type: String, value: 'CONTRACT#${customerId}#${status}', hidden: false },
            GSI4SK: { type: String, value: '${contractNo}', hidden: false },
            GSI5PK: { type: String, value: 'CONTRACT#${customerId}#${paymentStatus}', hidden: false },
            GSI5SK: { type: String, value: '${contractNo}', hidden: false },
            GSI6PK: { type: String, value: 'CONTRACT#${areaId}', hidden: false },
            GSI6SK: { type: String, value: '${contractNo}', hidden: false },
        },
        Payment: {
            PK: { type: String, value: 'PAYMENT', hidden: false },
            SK: { type: String, value: '${paymentId}', hidden: false },
            paymentId: { type: String, generate: 'ulid' },
            paymentDate: { type: String },
            paymentAmount: { type: Number },
            customerId: { type: String },
            customerName: { type: String },
            receiptNo: { type: String },
            activityLogs: { type: Array },
            forApprovalVersion: { type: Object },
            contractPayment: { type: Boolean, required: false },
            status: {
                type: String,
                enum: ['ACTIVE', 'INACTIVE', 'FOR_APPROVAL', 'FOR_DELETION', 'FOR_DEACTIVATION', 'NEW_RECORD', 'DRAFT'],
                required: false,
            },
            contractId: { type: String },
            contractName: { type: String },
            contractNo: { type: String },
            chequeClearStatus: { type: String, enum: ['PENDING', 'CLEARED'], required: false },
            paymentDetails: { type: Array }, //will hold 1 to many payment details , cheque no , cheque date , bank or cash payment
            paymentInvoiceDetails: { type: Array }, // will hold invoice to apply the payment
            changeReason: { type: String, required: false },
            approverMessage: { type: String, required: false },
            GSI1PK: { type: String, value: 'PAYMENT', hidden: false },
            GSI1SK: { type: String, value: '${receiptNo}', hidden: false },
            GSI2PK: { type: String, value: 'PAYMENT#${status}', hidden: false },
            GSI2SK: { type: String, value: '${receiptNo}', hidden: false },
            GSI3PK: { type: String, value: 'PAYMENT#${customerId}', hidden: false },
            GSI3SK: { type: String, value: '${receiptNo}', hidden: false },
            GSI4PK: { type: String, value: 'PAYMENT#${customerId}#${status}', hidden: false },
            GSI4SK: { type: String, value: '${receiptNo}', hidden: false },
            GSI5PK: { type: String, value: 'PAYMENT', hidden: false },
            GSI5SK: { type: String, value: '${paymentDate}', hidden: false },
            GSI6PK: { type: String, value: 'PAYMENT#${contractId}', hidden: false },
            GSI6SK: { type: String, value: '${paymentDate}', hidden: false },
        },
        ReturnGoodSold: {
            PK: { type: String, value: 'RETURN_GOOD_SOLD', hidden: false },
            SK: { type: String, value: '${returnGoodSoldId}', hidden: false },
            returnGoodSoldId: { type: String, generate: 'ulid' },
            invoiceId: { type: String },
            customerId: { type: String },
            customerName: { type: String },
            areaId: { type: String },
            areaName: { type: String },
            invoiceDocno: { type: String },
            rgsDocno: { type: String },
            activityLogs: { type: Array },
            dateReturned: { type: String },
            forApprovalVersion: { type: Object },
            approverMessage: { type: String, required: false },
            status: {
                type: String,
                enum: ['ACTIVE', 'INACTIVE', 'FOR_APPROVAL', 'FOR_DELETION', 'FOR_DEACTIVATION', 'NEW_RECORD', 'DRAFT'],
                required: false,
            },
            changeReason: { type: String, required: false },
            originalInvoiceDetails: { type: Array },
            modifiedInvoiceDetails: { type: Array },
            GSI1PK: { type: String, value: 'RETURN_GOOD_SOLD', hidden: false },
            GSI1SK: { type: String, value: '${rgsDocno}', hidden: false },
            GSI2PK: { type: String, value: 'RETURN_GOOD_SOLD#${status}', hidden: false },
            GSI2SK: { type: String, value: '${dateReturned}', hidden: false },
            GSI3PK: { type: String, value: 'RETURN_GOOD_SOLD#${invoiceId}', hidden: false },
            GSI3SK: { type: String, value: '${dateReturned}', hidden: false },
            GSI4PK: { type: String, value: 'RETURN_GOOD_SOLD#${invoiceId}#${status}', hidden: false },
            GSI4SK: { type: String, value: '${dateReturned}', hidden: false },
            GSI5PK: { type: String, value: 'RETURN_GOOD_SOLD#${customerId}', hidden: false },
            GSI5SK: { type: String, value: '${dateReturned}', hidden: false },
            GSI6PK: { type: String, value: 'RETURN_GOOD_SOLD#${areaId}', hidden: false },
            GSI6SK: { type: String, value: '${dateReturned}', hidden: false },
            // GSI7: Combined status + rgsDocno index for filtering by status with rgsDocno prefix search
            GSI7PK: { type: String, value: 'RETURN_GOOD_SOLD#${status}', hidden: false },
            GSI7SK: { type: String, value: '${rgsDocno}', hidden: false },
        },
        CollectionReceiptRange: {
            PK: { type: String, value: 'COLLECTION_RECEIPT_RANGE', hidden: false },
            SK: { type: String, value: '${collectionReceiptRangeId}', hidden: false },
            collectionReceiptRangeId: { type: String, generate: 'ulid' },
            areaId: { type: String },
            areaName: { type: String },
            startNumber: { type: Number },
            endNumber: { type: Number },
            lastUsedNumber: { type: Number, required: false },
            rangeStatus: {
                type: String,
                enum: ['AVAILABLE', 'ALL_USED_UP', 'CANCELLED'],
                required: false,
            },
            cancelledReceiptNumbers: { type: Array, required: false }, // Array of objects: { receiptNumber, cancellationReason, cancelledBy, cancelledAt, paymentId }
            activityLogs: { type: Array },
            status: {
                type: String,
                enum: ['ACTIVE', 'INACTIVE', 'FOR_APPROVAL', 'FOR_DELETION', 'FOR_DEACTIVATION', 'NEW_RECORD', 'DRAFT'],
                required: false,
            },
            forApprovalVersion: { type: Object },
            approverMessage: { type: String, required: false },
            changeReason: { type: String, required: false },
            GSI1PK: { type: String, value: 'COLLECTION_RECEIPT_RANGE', hidden: false },
            GSI1SK: { type: String, value: '${collectionReceiptRangeId}', hidden: false },
            GSI2PK: { type: String, value: 'COLLECTION_RECEIPT_RANGE#${rangeStatus}', hidden: false },
            GSI2SK: { type: String, value: '${areaId}', hidden: false },
            GSI3PK: { type: String, value: 'COLLECTION_RECEIPT_RANGE#${areaId}', hidden: false },
            GSI3SK: { type: String, value: '${startNumber}', hidden: false },
        },
    } as const,
    params: {
        isoDates: true,
        timestamps: true,
    },
};

export type SalesTypeDataType = Entity<typeof InvoicingSchema.models.SalesType>;
export type TerritoryManagerDataType = Entity<typeof InvoicingSchema.models.TerritoryManager>;
export type InvoiceDataType = Entity<typeof InvoicingSchema.models.Invoice>;
export type ContractDataType = Entity<typeof InvoicingSchema.models.Contract>;
export type PaymentDataType = Entity<typeof InvoicingSchema.models.Payment>;
export type ReturnGoodSoldDataType = Entity<typeof InvoicingSchema.models.ReturnGoodSold>;
export type CollectionReceiptRangeDataType = Entity<typeof InvoicingSchema.models.CollectionReceiptRange>;
