...
async function createSwaggerConfig() {
    return new DocumentBuilder()
        .setTitle('CONFIGURATION-API-SERVICE')
        .setDescription('CONFIGURATION-API-SERVICE API')
        .setVersion('1.0')
        .addBearerAuth(
            {
                type: 'http',
...
async function bootstrapServer() {
    ...
    Logger.log(`🚀 CONFIGURATION-API-SERVICE is running on: http://localhost:${port}/api`);
    Logger.log(`🚀 CONFIGURATION-API-SERVICE Swagger Endpoint : http://localhost:${port}/api/swagger`);
}
