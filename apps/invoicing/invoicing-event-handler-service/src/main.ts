...
async function createSwaggerConfig() {
    return new DocumentBuilder()
        .setTitle('INVOICING-EVENT-HANDLER-SERVICE')
        .setDescription('INVOICING-EVENT-HANDLER-SERVICE API')
        .setVersion('1.0')
        .addBearerAuth(
            {
                type: 'http',
...
async function bootstrapServer() {
    ...
    if (process.env.LOCALSTACK_STATUS === 'ENABLED') {
        ...
        Logger.log(`🚀 INVOICING-EVENT-HANDLER-SERVICE is running on: http://localhost:${port}/api`);
        Logger.log(`🚀 INVOICING-EVENT-HANDLER-SERVICE Swagger Endpoint : http://localhost:${port}/api/swagger`);
    }
}
