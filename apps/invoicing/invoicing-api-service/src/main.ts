...
async function createSwaggerConfig() {
    return new DocumentBuilder()
        .setTitle('INVOICING-API-SERVICE')
        .setDescription('INVOICING-API-SERVICE API')
        .setVersion('1.0')
        .addBearerAuth(
            {
                type: 'http',
...
async function bootstrapServer() {
    ...
    Logger.log(`🚀 INVOICING-API-SERVICE Swagger Endpoint : http://localhost:${port}/api/swagger`);
}
