...
async function createSwaggerConfig() {
    return new DocumentBuilder()
        .setTitle('ENVIRONMENT-INITIALIZER-API-SERVICE')
        .setDescription('ENVIRONMENT-INITIALIZER-API-SERVICE API')
        .setVersion('1.0')
        .addBearerAuth(
            {
                type: 'http',
...
async function bootstrapServer() {
    ...
    Logger.log(`🚀 ENVIRONMENT-INITIALIZER-API-SERVICE is running on: http://localhost:${port}/api`);
    Logger.log(`🚀 ENVIRONMENT-INITIALIZER-API-SERVICE Swagger Endpoint : http://localhost:${port}/api/swagger`);
}
