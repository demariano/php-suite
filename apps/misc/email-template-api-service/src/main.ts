...
async function createSwaggerConfig() {
    return new DocumentBuilder()
        .setTitle('EMAIL-TEMPLATE-API-SERVICE')
        .setDescription('EMAIL-TEMPLATE-API-SERVICE API')
        .setVersion('1.0')
        .addBearerAuth(
            {
                type: 'http',
...
async function bootstrapServer() {
    ...
    Logger.log(`🚀 EMAIL-TEMPLATE-API-SERVICE is running on: http://localhost:${port}/api`);
    Logger.log(`🚀 EMAIL-TEMPLATE-API-SERVICE Swagger Endpoint : http://localhost:${port}/api/swagger`);
}
