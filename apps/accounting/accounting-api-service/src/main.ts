...
async function createSwaggerConfig() {
    return new DocumentBuilder()
        .setTitle('ACCOUNTING-API-SERVICE')
        .setDescription('ACCOUNTING-API-SERVICE API')
        .setVersion('1.0')
        .addBearerAuth(
            {
                type: 'http',
...
async function bootstrapServer() {
    ...
    Logger.log(`🚀 ACCOUNTING-API-SERVICE is running on: http://localhost:${port}/api`);
    Logger.log(`🚀 ACCOUNTING-API-SERVICE Swagger Endpoint : http://localhost:${port}/api/swagger`);
}
