 ...
 * Note: Activity can be multiline (e.g., contains "Modified Fields: •...")
 */
export const parseActivityLog = (log: string): ParsedActivityLog => {
    ...
    // Use [\s\S]* instead of .+ to capture multiline content
    const dateTimeMatch = log.match(/^Date:\s*([^,]+,\s*[^,]+),\s*([\s\S]+)$/);
    if (dateTimeMatch) {
    ...
    const dateMatch = log.match(/^Date:\s*(.+?),\s*([\s\S]+)$/);
    if (dateMatch) {
        return {
                    ...
                    {activityLogs.map((log, index) => {
                        ...
                        return (
                            <tr key={index} className={`${activityStyle.bgColor} hover:opacity-80 transition-opacity`}>
                                <td
                                    className={`px-4 py-3 text-sm font-medium whitespace-nowrap ${activityStyle.textColor}`}
                                >
                                    {parsed.date || '-'}
                                </td>
                                <td className={`px-4 py-3 text-sm ${activityStyle.textColor}`}>{parsed.activity}</td>
                            </tr>
                        );
                    })}
                </tbody>
