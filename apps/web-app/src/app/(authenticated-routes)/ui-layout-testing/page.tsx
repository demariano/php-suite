...
const generateDummyData = (): SalesOrder[] => {
    ...
    const orders: SalesOrder[] = [
        ...
        },
        {
            ...
            companyName: 'Gaspar Antunes',
            status: 'FULFILLED',
            total: 2674.56,
            created: new Date('2019-05-10'),
            lastUpdated: yesterday,
        },
        {
            id: '4',
            orderNumber: 'SO-00004',
            companyName: 'Trienke van Aartsen',
            status: 'CONFIRMED',
            total: 1478.48,
            created: new Date('2019-05-12'),
            lastUpdated: new Date('2019-06-12'),
        },
        {
            id: '5',
        ...
        {
            id: '14',
            orderNumber: 'SO-00014',
            companyName: 'Network Systems',
            status: 'FULFILLED',
            total: 1234.56,
...
const generateSalesOrderDetail = (): SalesOrderDetail => {
    ...
    return {
        ...
        changeReason:
            'Customer requested status update and shipping address change. Total amount increased due to additional items.',
        activityLogs: [
...
export default function UILayoutTestingPage() {
    ...
    const [sortColumn, setSortColumn] = useState<
        'orderNumber' | 'companyName' | 'status' | 'total' | 'created' | 'lastUpdated'
    >('orderNumber');
    ...
    const StatusBadge = ({ status }: { status: SalesOrder['status'] }) => {
        ...
        return (
            <span
                className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md ${config.bg} ${config.text} text-xs font-semibold`}
            >
                ...
                <table className="w-full border-collapse">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {paginatedOrders.map((order) => {
                            return (
                                <tr key={order.id} className="transition-all duration-200 bg-white hover:bg-gray-50">
                                    <td className="px-6 py-5 text-sm font-medium text-gray-900">
                                        #{order.orderNumber}
                                    </td>
                                    <td className="px-6 py-5 text-sm font-medium text-gray-900">{order.companyName}</td>
                                    ...
                                    <td className="px-6 py-5 text-sm text-gray-600">{formatDate(order.created)}</td>
                                    ...
                                    {salesOrderDetail.changeReason && (
                                        <div className="bg-gradient-to-br from-teal-50 to-cyan-50 border-2 border-teal-400 rounded-xl p-5 mb-6 shadow-lg">
                                            <div className="flex items-center gap-3 mb-4">
                                                ...
                                                <h4 className="text-base font-bold text-teal-900">Change Reason</h4>
                                            </div>
                                                    ...
                                                    <div className="relative">
                                                        <input
                                                            ...
                                                            value={formatCurrency(
                                                                salesOrderDetail.forApprovalVersion.total
                                                            )}
                                                            readOnly
                                            ...
                                            {salesOrderDetail.activityLogs.map((log, index) => (
                                                <div
                                                    ...
                                                    className={`py-3 px-4 rounded-lg mb-2 transition-all duration-200 hover:bg-white/50 hover:shadow-sm ${
                                                        index < salesOrderDetail.activityLogs.length - 1
