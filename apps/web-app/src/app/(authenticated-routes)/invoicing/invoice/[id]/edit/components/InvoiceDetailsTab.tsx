...
export default function InvoiceDetailsTab({
    ...
    const handleAddDetailRecord = async () => {
        ...
        try {
            ...
            if (
                stockSelection.selectedProductDeal &&
            ...
            if (!currentStock || (currentStock.totalQuantity || 0) < totalQuantityNeeded) {
                setFlashNotification({
                    title: 'Insufficient Stock',
                    message: `Not enough stock available. Required: ${totalQuantityNeeded}, Available: ${
            ...
            const hasDuplicate = (formData.invoiceDetails || []).some(
                (d) =>
            ...
            if (remainingStock < 10 && remainingStock >= 0) {
                setFlashNotification({
                    ...
                    message: `Low stock: only ${remainingStock} remaining for ${foundStock.productName}`,
                    alertType: 'warning',
                });
            }
            ...
            if (onAutoSave) {
                try {
                    ...
                    const savedInvoiceId = await onAutoSave(updatedInvoice, currentInvoiceId);
                    if (savedInvoiceId && !currentInvoiceId) {
                        // First time save, invoice ID returned
                        setFlashNotification({
                            title: 'Draft Saved',
    ...
    const handleDeleteDetail = async (index: number) => {
        ...
        if (detailToDelete.invoiceDetailType === InvoiceDetailTypeEnum.REGULAR_ITEM && detailToDelete.productDealId) {
        ...
        if (associatedFreeItemIndex > -1) {
            ...
            const adjustedIndex =
                associatedFreeItemIndex > index ? associatedFreeItemIndex - 1 : associatedFreeItemIndex;
        ...
        // Auto-save draft invoice after deletion
        if (onAutoSave) {
            try {
                const updatedInvoice = {
                    ...formData,
                ...
                await onAutoSave(updatedInvoice, currentInvoiceId);
                setFlashNotification({
                    title: 'Draft Saved',
                    message: 'Invoice saved as draft',
                    alertType: 'success',
                            ...
                            <input
                                ...
                                value={
                                    stockSelection.selectedStock
                                        ? `${stockSelection.selectedStock.productName || ''} - ${
                                              stockSelection.selectedStock.productUnitName || ''
                                          } - ${stockSelection.selectedStock.stockTypeName || ''}${
                                              stockSelection.selectedStock.lotNo
                                                  ? ` (Lot: ${stockSelection.selectedStock.lotNo})`
                                                  : ''
                                          }`
                                        : ''
                                }
                                readOnly
                                placeholder={
                                    formData.productPriceTypeId
                                        ? 'Click to select stock item'
                                        : 'Select Product Price Type first'
                                }
                                onClick={() => {
                                    ...
                                    onChange={(e) => {
                                        ...
                                        const filteredDeals =
                                            stockSelection.selectedStock && contractProductDeals
                                                ? contractProductDeals.filter(
                                                      (deal) =>
                                                          deal.productId === stockSelection.selectedStock?.productId
                                                  )
                                                : [];
                                        const selectedDeal = filteredDeals.find(
                                            (deal) => deal.productDealId === dealId
                                        );
                                        setStockSelection((prev) => ({
                                            ...prev,
                                ...
                                >
                                    <option value="">
                                        {!stockSelection.selectedStock
                                            ...
                                            : !contractProductDeals || contractProductDeals.length === 0
                                    ...
                                    {stockSelection.selectedStock &&
                                        contractProductDeals
                                            ?.filter(
                                                (deal) => deal.productId === stockSelection.selectedStock?.productId
                                            )
                                            .map((deal) => (
                                                <option key={deal.productDealId} value={deal.productDealId}>
                                                    {deal.productDealName} (Min: {deal.minQty || 0}, Free:{' '}
