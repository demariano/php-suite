...
interface DenyReasonDialogProps {
    show: boolean;
    returnGoodSold: ReturnGoodSoldDto | null;
    onConfirm: (reason: string) => void;
    onCancel: () => void;
}

export default function DenyReasonDialog({ show, returnGoodSold, onConfirm, onCancel }: DenyReasonDialogProps) {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[1000] p-4">
            <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-lg space-y-6">
                <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-lg font-semibold text-gray-800 m-0">Deny Return Good Sold Changes</h3>
                </div>

                <p className="text-sm text-gray-600 leading-relaxed">
                    Please provide a reason for denying <strong>{returnGoodSold.rgsDocno || returnGoodSold.returnGoodSoldId}</strong>:
                </p>
