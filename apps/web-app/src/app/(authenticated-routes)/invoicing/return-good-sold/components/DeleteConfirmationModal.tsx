...
export default function DeleteConfirmationModal({ 
  ...
  if (!show || !returnGoodSold) return null;

  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[1000]">
      <div className="bg-white rounded-xl p-4 sm:p-6 w-full max-w-md mx-4 shadow-lg">
        <div className="flex items-center gap-3 mb-4">
          <div className="w-10 h-10 bg-red-50 rounded-full flex items-center justify-center text-red-600 text-xl">
            ⚠️
          </div>
          <h3 className="text-lg font-semibold text-gray-800 m-0">
            Delete Return Good Sold
          </h3>
        </div>

        <p className="text-sm text-gray-600 mb-6 leading-relaxed">
          Are you sure you want to delete <strong>{returnGoodSold?.rgsDocno}</strong>? This action cannot be undone.
        </p>
