...
export default function EditSalesTypePage({ params }: EditSalesTypePageProps) {
    ...
    return (
      <div className="p-4 sm:p-6 space-y-6">
        <div className="bg-red-50 border border-red-200 text-red-600 px-4 py-3 rounded-lg flex justify-between items-center shadow-sm">
          <span>Sales Type not found</span>
        </div>
      </div>
    );
  }
  ...
  const renderApprovalTab = () => {
    if (!selectedSalesType) return null;

    if (selectedSalesType.status === StatusEnum.FOR_DELETION) {
      return (
        <div className="space-y-6 animate-fadeIn">
          <div className="rounded-xl border-2 border-red-300 bg-red-50 p-6 shadow-sm sm:p-8">
            ...
            {selectedSalesType.changeReason && (
              <div className="mt-6 rounded-lg border-2 border-red-200 bg-white p-4">
                ...
                <p className="mt-2 whitespace-pre-wrap font-mono text-sm text-gray-600 leading-relaxed">{selectedSalesType.changeReason}</p>
              </div>
            )}
          </div>
    ...
    if (!selectedSalesType.forApprovalVersion) return null;
    ...
    const renderReadOnlyCheckbox = (label: string, value: boolean, colorClass: string, fieldName?: string) => {
      const fieldChanged = fieldName ? isFieldChanged(fieldName) : false;

      return (
        <div className="group">
          <label className={`block text-sm font-bold mb-2 flex items-center gap-2 ${
            fieldChanged ? 'text-gray-700' : 'text-gray-500'
          }`}>
            <input
              type="checkbox"
              checked={value}
              disabled
              className={`h-4 w-4 rounded border-2 text-blue-600 focus:ring-2 focus:ring-blue-500 transition-all duration-200 cursor-not-allowed ${
                fieldChanged
        ...
        <div className="space-y-4">
          <div className="rounded-xl border-2 border-gray-200 p-4 sm:p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {renderReadOnlyField('Sales Type Name', approvalData.salesTypeName, 'bg-blue-500', 'salesTypeName')}
              {renderReadOnlyField('Default Discount (%)', approvalData.defaultDiscount, 'bg-blue-500', 'defaultDiscount')}
              {renderReadOnlyField('Default Tax (%)', approvalData.defaultTax, 'bg-blue-500', 'defaultTax')}
            </div>
          </div>
          ...
          <div className="rounded-xl border-2 border-gray-200 p-4 sm:p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {renderReadOnlyCheckbox('Allow Discount', approvalData.allowDiscount as boolean, 'bg-blue-500', 'allowDiscount')}
              {renderReadOnlyCheckbox('Contract Sales', approvalData.contractSales as boolean, 'bg-blue-500', 'contractSales')}
              {renderReadOnlyCheckbox('Income Generating', approvalData.incomeGenerating as boolean, 'bg-blue-500', 'incomeGenerating')}
              {renderReadOnlyCheckbox('Taxable', approvalData.taxable as boolean, 'bg-blue-500', 'taxable')}
            </div>
          </div>
        </div>

        <div className="mt-8 flex flex-col gap-3 border-t-2 border-gray-200 pt-6 sm:flex-row sm:items-center sm:justify-between">
          {isAdminUser && selectedSalesType && ([StatusEnum.FOR_APPROVAL, StatusEnum.NEW_RECORD, StatusEnum.FOR_DELETION].includes(selectedSalesType.status as StatusEnum)) ? (
            <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
              <button
                type="button"
                onClick={handleDenyRecord}
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      ...
      {isLoading && !selectedSalesType && (
        <div className="flex min-h-96 items-center justify-center">
          <div className="text-gray-600">Loading sales type details...</div>
        </div>
      )}
              ...
              <div className="flex flex-nowrap gap-2">
                <button
                  ...
                  className={`flex-shrink-0 rounded-lg px-5 py-3 text-sm font-semibold transition-colors ${
                    getTabColorClasses(selectedSalesType.status || StatusEnum.ACTIVE, activeTab === 'details')
                  }`}
                >
                  <span className="flex items-center gap-2">
                    ...
                    {selectedSalesType && (
                      <>
                        ...
                        <span>{getStatusText(selectedSalesType.status || StatusEnum.ACTIVE)}</span>
                      </>
                    )}
                  </span>
                </button>

                {selectedSalesType.status !== StatusEnum.ACTIVE && (
                  <button
