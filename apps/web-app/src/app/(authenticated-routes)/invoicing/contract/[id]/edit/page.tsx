...
export default function EditContractPage({ params }: EditContractPageProps) {
    ...
    const [activeTab, setActiveTab] = useState<'details' | 'approval' | 'logs' | 'invoices' | 'payments'>('details');
    ...
    useEffect(() => {
        const fetchContract = async () => {
            ...
            try {
                ...
                if (
                    (contract.status === StatusEnum.FOR_APPROVAL ||
                        contract.status === StatusEnum.NEW_RECORD ||
                        contract.status === StatusEnum.FOR_DELETION) &&
                    isAdminUser
                ) {
    ...
    const renderApprovalTab = () => {
        ...
        return (
            <div className="space-y-6 animate-fadeIn rounded-xl border-2 border-blue-200 bg-white p-4 shadow-sm sm:p-6">
                ...
                <div className="space-y-4">
                    <div className="border-2 border-gray-200 rounded-xl p-4 sm:p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {renderReadOnlyField(
                                'Contract Number',
                                approvalData.contractNo,
                                'bg-blue-500',
                                'contractNo'
                            )}
                            {renderReadOnlyField(
                                'Contract Name',
                                approvalData.contractName,
                                'bg-blue-500',
                                'contractName'
                            )}
                            ...
                            {renderReadOnlyField(
                                'Contract Amount',
                                approvalData.contractAmount ? formatNumberWithCommas(approvalData.contractAmount) : '-',
                                'bg-blue-500',
                                'contractAmount'
                            )}
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                            {renderReadOnlyField(
                                'Start Date',
                                formatDate(approvalData.startDate),
                                'bg-blue-500',
                                'startDate'
                            )}
                            {renderReadOnlyField(
                                'End Date',
                                formatDate(approvalData.endDate),
                                'bg-blue-500',
                                'endDate'
                            )}
                        </div>
                ...
                {(() => {
                    const hasArrayChanges = (fieldName: string): boolean => {
                        ...
                        const originalStr = JSON.stringify(
                            original
                                .map((item: any) => ({ productId: item.productId, productDealId: item.productDealId }))
                                .sort()
                        );
                ...
                <div className="space-y-4">
                    <div className="border-2 border-gray-200 rounded-xl p-4 sm:p-6">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <h3 className="text-base font-bold text-blue-600">Status Information</h3>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {renderReadOnlyField(
                                'Payment Status',
                                approvalData.paymentStatus,
                                'bg-blue-500',
                                'paymentStatus'
                            )}
                            {renderReadOnlyField(
                                'Amount Paid',
                                formatNumberWithCommas(approvalData.totalAmountPaid),
                                'bg-blue-500',
                                'totalAmountPaid'
                            )}
                            {renderReadOnlyField(
                                'Delivery Status',
                                approvalData.deliveryStatus,
                                'bg-blue-500',
                                'deliveryStatus'
                            )}
                            {renderReadOnlyField(
                                'Delivered Amount',
                                formatNumberWithCommas(approvalData.deliveredAmount),
                                'bg-blue-500',
                                'deliveredAmount'
                            )}
                            {renderReadOnlyField(
                                'Invoiced Amount',
                                formatNumberWithCommas((approvalData as any).invoicedAmount),
                                'bg-blue-500',
                                'invoicedAmount'
                            )}
                        </div>
                ...
                {(approvalData.rebateType ||
                    approvalData.rebatePercentage ||
                    approvalData.rebateAmount ||
                    approvalData.rebateClaimedAmount ||
                    approvalData.rebateClaimedStatus) && (
                    <div className="space-y-4">
                        <div className="border-2 border-gray-200 rounded-xl p-4 sm:p-6">
                            <div className="flex items-center gap-3 mb-4">
                                <div className="p-2 bg-blue-600 rounded-lg shadow-md">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                {approvalData.rebateType === 'PERCENTAGE' &&
                                    renderReadOnlyField(
                                        'Rebate Percentage',
                                        approvalData.rebatePercentage
                                            ? formatNumberWithCommas(approvalData.rebatePercentage)
                                            : '-',
                                        'bg-blue-500',
                                        'rebatePercentage'
                                    )}
                                {approvalData.rebateType === 'AMOUNT' &&
                                    renderReadOnlyField(
                                        'Rebate Amount',
                                        approvalData.rebateAmount
                                            ? formatNumberWithCommas(approvalData.rebateAmount)
                                            : '-',
                                        'bg-blue-500',
                                        'rebateAmount'
                                    )}
                                {renderReadOnlyField(
                                    ...
                                    formatNumberWithCommas(approvalData.rebateClaimedAmount),
                                    'bg-blue-500',
                                    'rebateClaimedAmount'
                                )}
                                {renderReadOnlyField(
