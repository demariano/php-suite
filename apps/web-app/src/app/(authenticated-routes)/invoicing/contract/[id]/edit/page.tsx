...
export default function EditContractPage({ params }: EditContractPageProps) {
  ...
  if (!selectedContract && !isLoading) {
    return (
      <div className="p-6 text-center text-gray-600">
        Contract not found or could not be loaded.
        <button onClick={() => router.replace('/invoicing/contract')} className="mt-4 px-4 py-2 bg-blue-500 text-white rounded-md">
          Back to Contracts
        </button>
      </div>
  ...
  const approvalVersionData: ContractDto = {
    ...selectedContract,
  ...
  return (
    <div className="p-6 bg-gradient-to-br from-gray-50 to-white min-h-screen">
      ...
      <h1 className="text-3xl font-bold text-gray-900 mb-6">Edit Contract: {selectedContract?.contractNo}</h1>
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
        ...
        <div className="flex border-b-2 border-gray-200 mb-5 bg-gray-50 rounded-t-xl p-1">
          ...
          {selectedContract && (selectedContract.status === StatusEnum.FOR_APPROVAL || selectedContract.status === StatusEnum.NEW_RECORD || selectedContract.status === StatusEnum.FOR_DELETION) && (
            <button
        ...
        <div>
          ...
          {activeTab === 'approval' && !isLoading && selectedContract && (
            <div>
              <div className="mb-5">
                {(selectedContract.status === StatusEnum.FOR_APPROVAL || selectedContract.status === StatusEnum.NEW_RECORD || selectedContract.status === StatusEnum.FOR_DELETION) && (
                  <div className="bg-yellow-50 border border-yellow-200 rounded-md p-3 mb-4 flex items-center gap-2">
                    ...
                    <span className="text-yellow-800 text-sm">
                      {selectedContract.status === StatusEnum.FOR_DELETION ? 'This record is pending deletion approval.' : 'These are the proposed changes awaiting approval.'}
                    </span>
                  </div>
                )}
                  ...
                  <div className="bg-gray-50 border border-gray-200 rounded-xl p-5 mb-6 shadow-md">
                    ...
                    {Object.entries(approvalVersionData).map(([key, value]) => {
                      if (
                        ...
                        key === 'changeReason' || key === 'status' || key === 'contractId' ||
                        key === 'productDealQty' // Skip nested object
                      ) {
                        return null;
                      }

                      const originalValue = (selectedContract as any)[key];
                      ...
                      return (
                        <div key={key} className="mb-4">
                          ...
                          <div className={`p-2 border rounded-md text-sm ${isChanged ? 'bg-blue-50 border-blue-300' : 'bg-gray-100 border-gray-300'}`}>
                            ...
                            {isChanged && (
                              <span className="ml-2 text-xs text-blue-600">(Original: {typeof originalValue === 'object' ? JSON.stringify(originalValue) : String(originalValue)})</span>
                            )}
                          </div>
                        </div>
                      );
          ...
          {activeTab === 'logs' && !isLoading && selectedContract && (
            <div>
              <div className="mb-5">
                  ...
                  <div className="bg-gray-50 p-4 rounded-md border border-gray-200 max-h-72 overflow-y-auto">
                    {selectedContract.activityLogs.map((log, index) => (
                      <div
                        key={index}
                        className={`py-2 ${
                          index < selectedContract.activityLogs!.length - 1 ? 'border-b border-gray-200' : ''
                        }`}
                      >
                        {log}
                      </div>
