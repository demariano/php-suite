...
import { ContractApi, ContractTypeEnum, InvoiceApi, InvoiceDto, PaymentDto, PaymentInvoiceDetailsDto, useSessionStore } from '@data-access/index';
...
export default function PaymentInvoiceDetailsTab({
      ...
      if (formData.contractPayment && formData.contractId) {
        try {
          ...
          if (!fetchedContractType && contract) {
            console.warn(
              `Contract ${formData.contractId} (${contract.contractNo || contract.contractName || 'Unknown'}) does not have contractType set. ` +
          ...
          if (fetchedContractType === ContractTypeEnum.REGULAR && formData.paymentInvoiceDetails && formData.paymentInvoiceDetails.length > 0) {
            onFormDataChange({
  ...
  const handleApplySelectedInvoices = () => {
    ...
    const message = remainingPaymentAmount <= 0
  ...
  const handleUpdateAppliedAmount = (index: number, newAmount: number, skipValidation = false) => {
    ...
    if (!skipValidation && newTotal > formData.paymentAmount) {
      setFlashNotification({
        title: 'Validation Error',
        message: `Total applied amount ($${newTotal.toFixed(2)}) cannot exceed payment amount ($${formData.paymentAmount.toFixed(2)})`,
        alertType: 'error',
      });
      return;
    }
  ...
  const getTotalMismatchError = () => {
    ...
    if (difference > 0.01) {
      return `Total applied amount ($${totalApplied.toFixed(2)}) does not match payment amount ($${formData.paymentAmount.toFixed(2)})`;
    }
    return null;
  };
        ...
        <div className="bg-white rounded-lg p-4 mb-4 border border-gray-200">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <div className={`text-lg font-semibold ${getTotalMismatchError() ? 'text-red-600' : 'text-green-600'}`}>
                        ...
                        {isReadOnly ? (
                          ...
                          <div className="w-full max-w-[150px]">
                            <input
                              ...
                              type="text"
                              value={editingAmounts.has(index) 
                                ? editingAmounts.get(index) || ''
                                : detail.amountApplied.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                              onChange={(e) => {
                                ...
                                if (!isNaN(numValue) && rawValue !== '' && rawValue !== '.') {
                                  handleUpdateAppliedAmount(index, numValue, true); // Skip validation while typing
