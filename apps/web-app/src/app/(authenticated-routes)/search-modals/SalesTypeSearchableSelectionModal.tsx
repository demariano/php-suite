...
interface SalesTypeSearchableSelectionModalProps {
  show: boolean;
  title: string;
  selectedValue: string | null;
  onSelect: (salesType: SalesTypeDto) => void;
  onClose: () => void;
}
...
export default function SalesTypeSearchableSelectionModal({
  show,
  title,
  selectedValue,
  onSelect,
  onClose
}: SalesTypeSearchableSelectionModalProps) {
  ...
  const handleSelect = (item: Item) => {
    // Create a full SalesTypeDto object
    const salesType: SalesTypeDto = {
      salesTypeId: item.id,
      salesTypeName: item.name,
            ...
            <div className="bg-white rounded-2xl w-[520px] max-w-[90vw] max-h-[80vh] overflow-hidden shadow-2xl flex flex-col border border-gray-100">
                ...
                <div className="px-6 pb-4">
                    <div className="relative">
                        ...
                        <input
                            ...
                            placeholder="Search sales types by name..."
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                            ...
                            <span className="text-sm font-medium">
                                {searchTerm ? `No sales types matching "${searchTerm}"` : 'No sales types available'}
                            </span>
                            {searchTerm && (
                ...
                <div className="border-t border-gray-100 px-6 py-3 flex items-center justify-between">
                    <span className="text-xs text-gray-400">
                        {items.length > 0 ? `${items.length} ${items.length !== 1 ? 'sales types' : 'sales type'} shown` : ''}
                    </span>
