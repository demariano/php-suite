...
export default function ProductDealSearchableSelectionModal({
    ...
    const loadItems = async (searchQuery?: string) => {
        try {
            ...
            if (searchQuery?.trim()) {
                response = await ProductDealApi.getProductDealsByName(searchQuery.trim(), limit, direction, cursor);
            } else {
                response = await ProductDealApi.getProductDealsByStatus(limit, StatusEnum.ACTIVE, direction, cursor);
            }
    ...
    return (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-[1000] animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl w-[560px] max-w-[90vw] max-h-[80vh] overflow-hidden shadow-2xl flex flex-col border border-gray-100">
                {/* Header */}
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                            ...
                            <span className="text-sm font-medium">
                                {searchTerm
                                    ? `No product deals matching "${searchTerm}"`
                        ...
                        <div className="space-y-0.5">
                            {items.map((item) => {
                                const isSelected = selectedValue === item.productDealId;
                                return (
                                    <button
                                    ...
                                    >
                                        <div className="flex items-center gap-3 min-w-0">
                                            ...
                                            >
                                                {item.productDealName?.toString()?.charAt(0)?.toUpperCase() || '?'}
                                            </div>
                                            <div className="min-w-0">
                                                ...
                                                <div className="flex items-center gap-2 flex-wrap mt-0.5 text-xs text-gray-400">
                                                    ...
                                                    {item.additionalQty !== undefined && (
                                                        <span>Additional Qty: {item.additionalQty}</span>
                                                    )}
                                                </div>
                                            </div>
                                        </div>
                ...
                <div className="border-t border-gray-100 px-6 py-3 flex items-center justify-between">
                    <span className="text-xs text-gray-400">
                        {items.length > 0 ? `${items.length} ${items.length !== 1 ? 'deals' : 'deal'} shown` : ''}
                    </span>
