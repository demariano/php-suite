...
import { ProductApi, ProductCategoryDto } from '@data-access/index';
import { useCallback, useEffect, useMemo, useRef, useState } from 'react';

interface ProductCategoryMultiSearchableSelectionModalProps {
    show: boolean;
    title: string;
    selectedValues?: string[];
    onSelectMultiple?: (categories: ProductCategoryDto[]) => void;
    onClose?: () => void;
}
...
export default function ProductCategoryMultiSearchableSelectionModal({
    show,
    title,
    selectedValues,
    onSelectMultiple,
    onClose,
}: ProductCategoryMultiSearchableSelectionModalProps) {
    ...
    const [selectedItems, setSelectedItems] = useState<ProductCategoryDto[]>([]);
    ...
    useEffect(() => {
        ...
        const hydrateSelected = async () => {
            ...
            const results = await Promise.allSettled(ids.map((id) => ProductApi.getProductCategoryById(id)));
            if (cancelled) return;

            const hydrated: ProductCategoryDto[] = results.map((res, idx) => {
                const fallbackId = ids[idx];
                if (res.status !== 'fulfilled') {
                    return { productCategoryId: fallbackId, productCategoryName: fallbackId };
                }

                const value = res.value as unknown;
                const maybeAxios = value as { data?: ProductCategoryDto };
                const category = maybeAxios?.data ?? (value as ProductCategoryDto);
                const productCategoryId = category?.productCategoryId || fallbackId;
                const productCategoryName = category?.productCategoryName || productCategoryId;
                return { ...category, productCategoryId, productCategoryName };
            });
    ...
    const loadItems = useCallback(
        async (searchQuery?: string) => {
            ...
            try {
                ...
                let response: ProductCategoriesResponse;
                if (effectiveQuery) {
                    ...
                    response = await ProductApi.getProductCategoriesByStatus(
                        limit,
                        'ACTIVE',
                        direction,
                        cursor,
                ...
                const list: Item[] = Array.isArray(response?.data)
                    ? response.data.map((c: ProductCategoryDto) => ({
                          id: c.productCategoryId || '',
                          name: c.productCategoryName || c.productCategoryId || '',
                          fullData: c,
                      }))
                    : [];
    ...
    const selectedIds = useMemo(() => new Set(selectedItems.map((c) => c.productCategoryId)), [selectedItems]);

    const handleToggle = (item: Item) => {
        const category: ProductCategoryDto = item.fullData ?? {
            productCategoryId: item.id,
            productCategoryName: item.name,
        };
        const exists = selectedItems.some((c) => c.productCategoryId === category.productCategoryId);
        if (exists) {
            setSelectedItems(selectedItems.filter((c) => c.productCategoryId !== category.productCategoryId));
        } else {
            ...
            <div className="bg-white rounded-2xl w-[520px] max-w-[90vw] max-h-[80vh] overflow-hidden shadow-2xl flex flex-col border border-gray-100">
                ...
                <div className="px-6 pb-3">
                    <p className="text-xs text-gray-500">
                        {selectedItems.length === 0
                            ? 'No categories selected (All Categories)'
                ...
                <div className="flex-1 overflow-y-auto px-3 py-2">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                            ...
                            <span className="text-sm">Loading product categories...</span>
                        </div>
                    ) : items.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                            <svg
                            ...
                            <span className="text-sm font-medium">
                                {debouncedSearchTerm
                                    ? `No categories matching "${debouncedSearchTerm}"`
                ...
                <div className="border-t border-gray-100 px-6 py-3 flex items-center justify-between">
                    <span className="text-xs text-gray-400">
                        {items.length > 0 ? `${items.length} categor${items.length !== 1 ? 'ies' : 'y'} shown` : ''}
                    </span>
