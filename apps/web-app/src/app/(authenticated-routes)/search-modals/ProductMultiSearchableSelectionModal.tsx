...
interface ProductMultiSearchableSelectionModalProps {
    show: boolean;
    title: string;
    selectedValues?: string[];
    onSelectMultiple?: (products: ProductDto[]) => void;
    onClose?: () => void;
}
...
export default function ProductMultiSearchableSelectionModal({
    show,
    title,
    selectedValues,
    onSelectMultiple,
    onClose,
}: ProductMultiSearchableSelectionModalProps) {
    ...
    useEffect(() => {
        ...
        const hydrateSelected = async () => {
            ...
            const hydrated: ProductDto[] = results.map((res, idx) => {
                const fallbackId = ids[idx];
                if (res.status !== 'fulfilled') {
                    return { productId: fallbackId, productName: fallbackId };
                }

                const value = res.value as unknown;
                const maybeAxios = value as { data?: ProductDto };
                const product = maybeAxios?.data ?? (value as ProductDto);
                const productId = product?.productId || fallbackId;
        ...
        async (searchQuery?: string) => {
            ...
            try {
                ...
                const list: Item[] = Array.isArray(response?.data)
                    ? response.data.map((p: ProductDto) => ({
                          id: p.productId || '',
    ...
    const selectedIds = useMemo(() => new Set(selectedItems.map((p) => p.productId)), [selectedItems]);

    const handleToggle = (item: Item) => {
        const product: ProductDto = item.fullData ?? { productId: item.id, productName: item.name };
        const exists = selectedItems.some((p) => p.productId === product.productId);
        if (exists) {
            setSelectedItems(selectedItems.filter((p) => p.productId !== product.productId));
        } else {
            ...
            <div className="bg-white rounded-2xl w-[520px] max-w-[90vw] max-h-[80vh] overflow-hidden shadow-2xl flex flex-col border border-gray-100">
                ...
                <div className="px-6 pb-3">
                    <p className="text-xs text-gray-500">
                        {selectedItems.length === 0
                            ? 'No products selected (All Products)'
                ...
                <div className="border-t border-gray-100 px-6 py-3 flex items-center justify-between">
                    <span className="text-xs text-gray-400">
                        {items.length > 0 ? `${items.length} product${items.length !== 1 ? 's' : ''} shown` : ''}
                    </span>
