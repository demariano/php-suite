...
export default function TerritoryManagerSearchableSelectionModal({
  show,
  title,
  selectedValue,
  onSelect,
  onClose
}: TerritoryManagerSearchableSelectionModalProps) {
  ...
  const loadItems = async (searchQuery?: string) => {
    ...
    try {
      ...
      const itemsList = response.data.map((item: any) => ({
        id: item.territoryManagerId,
        name: item.territoryManagerName,
      ...
      console.error('Error loading territory managers:', error);
    } finally {
      setLoading(false);
    }
  };
            ...
            <div className="bg-white rounded-2xl w-[520px] max-w-[90vw] max-h-[80vh] overflow-hidden shadow-2xl flex flex-col border border-gray-100">
                ...
                <div className="px-6 pb-4">
                    <div className="relative">
                        ...
                        <input
                            ...
                            placeholder="Search territory managers by name..."
                ...
                <div className="flex-1 overflow-y-auto px-3 py-2">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                            ...
                            <span className="text-sm">Loading territory managers...</span>
                        </div>
                    ) : items.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                            ...
                            <span className="text-sm font-medium">
                                {searchTerm ? `No territory managers matching "${searchTerm}"` : 'No territory managers available'}
                            </span>
                            {searchTerm && (
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center gap-2 flex-wrap mt-0.5 text-xs text-gray-400">
                                            {item.contactNo && <span>Contact: {item.contactNo}</span>}
                                        </div>
                                            </div>
                                        </div>
                                        {isSelected && (
                ...
                <div className="border-t border-gray-100 px-6 py-3 flex items-center justify-between">
                    <span className="text-xs text-gray-400">
                        {items.length > 0 ? `${items.length} ${items.length !== 1 ? 'territory managers' : 'territory manager'} shown` : ''}
                    </span>
