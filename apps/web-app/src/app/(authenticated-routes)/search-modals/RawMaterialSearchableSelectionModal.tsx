...
import { RawMaterialApi, RawMaterialDto } from '@data-access/index';
import { useEffect, useState } from 'react';

interface RawMaterialSearchableSelectionModalProps {
    show: boolean;
    title: string;
    selectedValue: string | null;
    onSelect: (rawMaterial: RawMaterialDto) => void;
    onClose: () => void;
}
...
export default function RawMaterialSearchableSelectionModal({
    show,
    title,
    selectedValue,
    onSelect,
    onClose,
}: RawMaterialSearchableSelectionModalProps) {
    ...
    const loadItems = async (searchQuery?: string) => {
        ...
        try {
            ...
            if (searchQuery) {
                response = await RawMaterialApi.searchRawMaterialsByName(searchQuery, limit, direction, cursor);
            } else {
                response = await RawMaterialApi.getRawMaterialsByStatus(limit, 'ACTIVE', direction, cursor);
            }

            const itemsList = response.data.map((item: any) => ({
                id: item.rawMaterialId,
                name: item.rawMaterialName,
                unitName: item.rawMaterialUnitName,
            }));
    ...
    const handleSelect = (item: Item) => {
        const rawMaterial = {
            ...
            rawMaterialUnitName: item.unitName,
            ...
            <div className="bg-white rounded-2xl w-[520px] max-w-[90vw] max-h-[80vh] overflow-hidden shadow-2xl flex flex-col border border-gray-100">
                ...
                <div className="flex-1 overflow-y-auto px-3 py-2">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                            ...
                            <span className="text-sm">Loading raw materials...</span>
                        </div>
                    ) : items.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                            ...
                            <span className="text-sm font-medium">
                                {searchTerm ? `No raw materials matching "${searchTerm}"` : 'No raw materials available'}
                            </span>
                            {searchTerm && (
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center gap-2 flex-wrap mt-0.5 text-xs text-gray-400">
                                            {item.unitName && <span>Unit: {item.unitName}</span>}
                                        </div>
                                            </div>
                                        </div>
                                        {isSelected && (
                ...
                <div className="border-t border-gray-100 px-6 py-3 flex items-center justify-between">
                    <span className="text-xs text-gray-400">
                        {items.length > 0 ? `${items.length} ${items.length !== 1 ? 'raw materials' : 'raw material'} shown` : ''}
                    </span>
