            ...
            <div className="bg-white rounded-2xl w-[640px] max-w-[90vw] max-h-[80vh] overflow-hidden shadow-2xl flex flex-col border border-gray-100">
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                            ...
                            <span className="text-sm font-medium">
                                {searchTerm ? `No ACTIVE invoices matching "${searchTerm}"` : 'No ACTIVE invoices available'}
                            </span>
                            {searchTerm && (
                                    ...
                                    >
                                        <div className="flex items-center gap-3 min-w-0">
                                            <div className={`w-7 h-7 rounded-md flex items-center justify-center text-xs font-bold shrink-0 ${
                                                ...
                                                {(item.docno)?.toString()?.charAt(0)?.toUpperCase() || '?'}
                                            </div>
                                            <div className="min-w-0">
                                                <span className="font-medium block truncate">{item.docno}</span>
                                        <div className="flex items-center gap-2 flex-wrap mt-0.5 text-xs text-gray-400">
                                            ...
                                            {item.invoiceDate && <span>Date: {item.invoiceDate}</span>}
                                            {item.invoiceAmount !== undefined && <span>Amount: \u20B1{item.invoiceAmount.toFixed(2)}</span>}
                                        </div>
                                            </div>
                                        </div>
                                        {isSelected && (
                ...
                <div className="border-t border-gray-100 px-6 py-3 flex items-center justify-between">
                    <span className="text-xs text-gray-400">
                        {items.length > 0 ? `${items.length} ${items.length !== 1 ? 'invoices' : 'invoice'} shown` : ''}
                    </span>
