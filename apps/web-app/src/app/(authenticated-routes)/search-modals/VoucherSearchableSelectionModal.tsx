...
import { VoucherApi, VoucherDto } from '@data-access/index';
import { useEffect, useState } from 'react';

interface VoucherSearchableSelectionModalProps {
  show: boolean;
  title: string;
  selectedValue: string | null;
  onSelect: (voucher: VoucherDto) => void;
  onClose: () => void;
}
...
export default function VoucherSearchableSelectionModal({
  show,
  title,
  selectedValue,
  onSelect,
  onClose
}: VoucherSearchableSelectionModalProps) {
  const [items, setItems] = useState<Item[]>([]);
  const [fullVouchers, setFullVouchers] = useState<VoucherDto[]>([]);
  ...
  const loadItems = async (searchQuery?: string) => {
    ...
    try {
      ...
      setItems(itemsList);
      setFullVouchers(response.data); // Store full voucher data
  ...
  const handleSelect = (item: Item) => {
    // Find the full voucher data from the stored full vouchers
    const fullVoucher = fullVouchers.find(voucher => voucher.voucherId === item.id);
    
    if (fullVoucher) {
      onSelect(fullVoucher);
    } else {
      // Fallback: create a basic VoucherDto if full data not found
      const voucher: VoucherDto = {
        ...
        chequeNo: '',
        chequeDate: '',
        totalAmount: item.voucherAmount,
        accountId: '',
            ...
            <div className="bg-white rounded-2xl w-[520px] max-w-[90vw] max-h-[80vh] overflow-hidden shadow-2xl flex flex-col border border-gray-100">
                ...
                <div className="px-6 pb-4">
                    <div className="relative">
                        ...
                        <input
                            ...
                            placeholder="Search vouchers by voucher number..."
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                            ...
                            <span className="text-sm font-medium">
                                {searchTerm ? `No vouchers matching "${searchTerm}"` : 'No vouchers available'}
                            </span>
                            {searchTerm && (
                                    ...
                                    >
                                        <div className="flex items-center gap-3 min-w-0">
                                            <div className={`w-7 h-7 rounded-md flex items-center justify-center text-xs font-bold shrink-0 ${
                                                ...
                                                {(item.voucherNo)?.toString()?.charAt(0)?.toUpperCase() || '?'}
                                            </div>
                                            <div className="min-w-0">
                                                <span className="font-medium block truncate">{item.voucherNo}</span>
                                        <div className="flex items-center gap-2 flex-wrap mt-0.5 text-xs text-gray-400">
                                            <span>Date: {item.voucherDate}</span>
                                            {item.accountName && <span>Account: {item.accountName}</span>}
                                            {item.voucherAmount !== undefined && <span>Amount: {item.voucherAmount.toLocaleString()}</span>}
                                        </div>
                                            </div>
                                        </div>
                                        {isSelected && (
                ...
                <div className="border-t border-gray-100 px-6 py-3 flex items-center justify-between">
                    <span className="text-xs text-gray-400">
                        {items.length > 0 ? `${items.length} ${items.length !== 1 ? 'vouchers' : 'voucher'} shown` : ''}
                    </span>
