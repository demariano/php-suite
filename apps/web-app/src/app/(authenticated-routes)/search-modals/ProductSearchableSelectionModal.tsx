...
export default function ProductSearchableSelectionModal({
    ...
    const loadItems = async (searchQuery?: string) => {
        try {
            ...
            if (searchQuery?.trim()) {
                response = await ProductApi.getProductsByName(searchQuery.trim(), limit, direction, cursor);
            } else {
                response = await ProductApi.getProductsByStatus(limit, StatusEnum.ACTIVE, direction, cursor);
            }
