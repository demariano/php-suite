...
import { ProductApi, ProductDealApi, ProductPriceTypeApi } from '@data-access/index';
import { useEffect, useState } from 'react';

interface ProductSearchableSelectionModalProps {
  show: boolean;
  title: string;
  type: 'category' | 'class' | 'deal' | 'price-type';
    ...
    try {
      ...
      const itemsList = response.data.map((item: any) => {
        switch (type) {
          ...
          case 'deal':
            return {
              id: item.productDealId,
              name: item.productDealName
            };
          case 'price-type':
            return {
          ...
          default:
            throw new Error(`Unsupported type for mapping: ${type}`);
        }
      });
