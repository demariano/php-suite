...
import { RawMaterialsLocationApi, RawMaterialsLocationDto } from '@data-access/index';
import { useEffect, useState } from 'react';

interface RawMaterialsLocationSearchableSelectionModalProps {
  show: boolean;
  title: string;
  selectedValue: string | null;
  onSelect: (location: RawMaterialsLocationDto) => void;
  onClose: () => void;
}
...
export default function RawMaterialsLocationSearchableSelectionModal({
  show,
  title,
  selectedValue,
  onSelect,
  onClose
}: RawMaterialsLocationSearchableSelectionModalProps) {
  ...
  const loadItems = async (searchQuery?: string) => {
    ...
    try {
      ...
      const itemsList = response.data.map((item: any) => ({
        id: item.rawMaterialsLocationId,
        name: item.rawMaterialsLocationName
      }));
      ...
      console.error('Error loading raw materials locations:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSelect = (item: Item) => {
    // Create a full RawMaterialsLocationDto object
    const location: RawMaterialsLocationDto = {
      rawMaterialsLocationId: item.id,
      rawMaterialsLocationName: item.name,
            ...
            <div className="bg-white rounded-2xl w-[520px] max-w-[90vw] max-h-[80vh] overflow-hidden shadow-2xl flex flex-col border border-gray-100">
                ...
                <div className="px-6 pb-4">
                    <div className="relative">
                        ...
                        <input
                            ...
                            placeholder="Search raw materials locations by name..."
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                            ...
                            <span className="text-sm font-medium">
                                {searchTerm ? `No raw materials locations matching "${searchTerm}"` : 'No raw materials locations available'}
                            </span>
                            {searchTerm && (
                ...
                <div className="border-t border-gray-100 px-6 py-3 flex items-center justify-between">
                    <span className="text-xs text-gray-400">
                        {items.length > 0 ? `${items.length} ${items.length !== 1 ? 'locations' : 'location'} shown` : ''}
                    </span>
