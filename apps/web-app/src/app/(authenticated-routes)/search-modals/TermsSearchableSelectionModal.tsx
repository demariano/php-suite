...
import { TermsApi, TermsDto } from '@data-access/index';
import { useEffect, useState } from 'react';

interface TermsSearchableSelectionModalProps {
  show: boolean;
  title: string;
...
export default function TermsSearchableSelectionModal({
  show,
  title,
  selectedValue,
  onSelect,
  onClose
}: TermsSearchableSelectionModalProps) {
  ...
  const loadItems = async (searchQuery?: string) => {
    ...
    try {
      ...
      if (searchQuery) {
        ...
        response = await TermsApi.getTermsByStatus(
          limit,
          'ACTIVE',
          direction,
          cursor
            ...
            <div className="bg-white rounded-2xl w-[520px] max-w-[90vw] max-h-[80vh] overflow-hidden shadow-2xl flex flex-col border border-gray-100">
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                            ...
                            <span className="text-sm font-medium">
                                {searchTerm ? `No terms matching "${searchTerm}"` : 'No terms available'}
                            </span>
                            {searchTerm && (
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center gap-2 flex-wrap mt-0.5 text-xs text-gray-400">
                                            {item.days !== undefined && <span>Days: {item.days}</span>}
                                        </div>
                                            </div>
                                        </div>
                                        {isSelected && (
                ...
                <div className="border-t border-gray-100 px-6 py-3 flex items-center justify-between">
                    <span className="text-xs text-gray-400">
                        {items.length > 0 ? `${items.length} ${items.length !== 1 ? 'terms' : 'term'} shown` : ''}
                    </span>
