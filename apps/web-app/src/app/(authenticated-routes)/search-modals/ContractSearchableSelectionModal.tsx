...
interface ContractSearchableSelectionModalProps {
  show: boolean;
  title: string;
  selectedValue: string | null;
  customerId?: string; // Add optional customerId
...
export default function ContractSearchableSelectionModal({
  show,
  title,
  selectedValue,
  customerId,
  onSelect,
  onClose
}: ContractSearchableSelectionModalProps) {
  const [items, setItems] = useState<Item[]>([]);
  const [fullContracts, setFullContracts] = useState<ContractDto[]>([]);
  ...
  const loadItems = async (searchQuery?: string) => {
    ...
    try {
      ...
      setItems(itemsList);
      setFullContracts(response.data); // Store full contract data
  ...
  const handleSelect = (item: Item) => {
    // Find the full contract data from the stored full contracts
    const fullContract = fullContracts.find(contract => contract.contractId === item.id);
    
    if (fullContract) {
      onSelect(fullContract);
    } else {
      // Fallback: create a basic ContractDto if full data not found
            ...
            <div className="bg-white rounded-2xl w-[520px] max-w-[90vw] max-h-[80vh] overflow-hidden shadow-2xl flex flex-col border border-gray-100">
                ...
                <div className="px-6 pb-4">
                    <div className="relative">
                        ...
                        <input
                            ...
                            placeholder="Search contracts by contract number..."
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                            ...
                            <span className="text-sm font-medium">
                                {searchTerm ? `No contracts matching "${searchTerm}"` : 'No contracts available'}
                            </span>
                            {searchTerm && (
                                    ...
                                    >
                                        <div className="flex items-center gap-3 min-w-0">
                                            <div className={`w-7 h-7 rounded-md flex items-center justify-center text-xs font-bold shrink-0 ${
                                                ...
                                                {(item.contractName)?.toString()?.charAt(0)?.toUpperCase() || '?'}
                                            </div>
                                            <div className="min-w-0">
                                                <span className="font-medium block truncate">{item.contractName}</span>
                                        <div className="flex items-center gap-2 flex-wrap mt-0.5 text-xs text-gray-400">
                                            {item.contractNo && <span>Contract #: {item.contractNo}</span>}
                ...
                <div className="border-t border-gray-100 px-6 py-3 flex items-center justify-between">
                    <span className="text-xs text-gray-400">
                        {items.length > 0 ? `${items.length} ${items.length !== 1 ? 'contracts' : 'contract'} shown` : ''}
                    </span>
