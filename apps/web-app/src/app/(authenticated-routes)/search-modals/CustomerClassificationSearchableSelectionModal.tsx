...
interface CustomerClassificationSearchableSelectionModalProps {
  show: boolean;
  title: string;
  selectedValue: string | null;
  onSelect: (classification: CustomerClassificationDto) => void;
  onClose: () => void;
}
...
export default function CustomerClassificationSearchableSelectionModal({
  show,
  title,
  selectedValue,
  onSelect,
  onClose
}: CustomerClassificationSearchableSelectionModalProps) {
  ...
  const loadItems = async (searchQuery?: string) => {
    ...
    try {
      ...
      const itemsList = response.data.map((item: any) => ({
        id: item.customerClassificationId,
        name: item.customerClassificationName
      }));
  ...
  const handleSelect = (item: Item) => {
    // Create a full CustomerClassificationDto object
    const classification: CustomerClassificationDto = {
            ...
            <div className="bg-white rounded-2xl w-[520px] max-w-[90vw] max-h-[80vh] overflow-hidden shadow-2xl flex flex-col border border-gray-100">
                ...
                <div className="px-6 pb-4">
                    <div className="relative">
                        ...
                        <input
                            ...
                            placeholder="Search classifications by name..."
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                            ...
                            <span className="text-sm font-medium">
                                {searchTerm ? `No classifications matching "${searchTerm}"` : 'No classifications available'}
                            </span>
                            {searchTerm && (
                ...
                <div className="border-t border-gray-100 px-6 py-3 flex items-center justify-between">
                    <span className="text-xs text-gray-400">
                        {items.length > 0 ? `${items.length} ${items.length !== 1 ? 'classifications' : 'classification'} shown` : ''}
                    </span>
