...
import { StockTypeApi, StockTypeDto } from '@data-access/index';
import { useCallback, useEffect, useMemo, useRef, useState } from 'react';

interface StockTypeMultiSearchableSelectionModalProps {
    show: boolean;
    title: string;
    selectedValues?: string[];
    onSelectMultiple?: (stockTypes: StockTypeDto[]) => void;
    onClose?: () => void;
}
...
interface Item {
    id: string;
    name: string;
    fullData?: StockTypeDto;
}

type StockTypesResponse = {
    data?: StockTypeDto[];
    nextCursorPointer?: string;
    prevCursorPointer?: string;
};

export default function StockTypeMultiSearchableSelectionModal({
    show,
    title,
    selectedValues,
    onSelectMultiple,
    onClose,
}: StockTypeMultiSearchableSelectionModalProps) {
    ...
    const [selectedItems, setSelectedItems] = useState<StockTypeDto[]>([]);
    ...
    useEffect(() => {
        ...
        const hydrateSelected = async () => {
            ...
            const results = await Promise.allSettled(ids.map((id) => StockTypeApi.getStockTypeById(id)));
            if (cancelled) return;

            const hydrated: StockTypeDto[] = results.map((res, idx) => {
                const fallbackId = ids[idx];
                if (res.status !== 'fulfilled') {
                    return { stockTypeId: fallbackId, stockTypeName: fallbackId };
                }

                const value = res.value as unknown;
                const maybeAxios = value as { data?: StockTypeDto };
                const stockType = maybeAxios?.data ?? (value as StockTypeDto);
                const stockTypeId = stockType?.stockTypeId || fallbackId;
                const stockTypeName = stockType?.stockTypeName || stockTypeId;
                return { ...stockType, stockTypeId, stockTypeName };
            });
    ...
    const loadItems = useCallback(
        async (searchQuery?: string) => {
            ...
            try {
                ...
                let response: StockTypesResponse;
                if (effectiveQuery) {
                    response = await StockTypeApi.getStockTypesByName(effectiveQuery, limit, direction, cursor);
                } else {
                    response = await StockTypeApi.getStockTypesByStatus(limit, 'ACTIVE', direction, cursor);
                }

                const list: Item[] = Array.isArray(response?.data)
                    ? response.data.map((s: StockTypeDto) => ({
                          id: s.stockTypeId || '',
                          name: s.stockTypeName || s.stockTypeId || '',
                          fullData: s,
                      }))
                    : [];
    ...
    const selectedIds = useMemo(() => new Set(selectedItems.map((s) => s.stockTypeId)), [selectedItems]);

    const handleToggle = (item: Item) => {
        const stockType: StockTypeDto = item.fullData ?? {
            stockTypeId: item.id,
            stockTypeName: item.name,
        };
        const exists = selectedItems.some((s) => s.stockTypeId === stockType.stockTypeId);
        if (exists) {
            setSelectedItems(selectedItems.filter((s) => s.stockTypeId !== stockType.stockTypeId));
        } else {
            setSelectedItems([...selectedItems, stockType]);
        }
    };
            ...
            <div className="bg-white rounded-2xl w-[520px] max-w-[90vw] max-h-[80vh] overflow-hidden shadow-2xl flex flex-col border border-gray-100">
                ...
                <div className="px-6 pb-3">
                    <p className="text-xs text-gray-500">
                        {selectedItems.length === 0
                            ? 'No stock types selected (All Stock Types)'
                            : `${selectedItems.length} stock type${selectedItems.length === 1 ? '' : 's'} selected`}
                    </p>
                </div>
                ...
                <div className="flex-1 overflow-y-auto px-3 py-2">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                            ...
                            <span className="text-sm">Loading stock types...</span>
                        </div>
                    ) : items.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                            <svg
                            ...
                            <span className="text-sm font-medium">
                                {debouncedSearchTerm
                                    ? `No stock types matching "${debouncedSearchTerm}"`
                                    : 'No stock types available'}
                            </span>
                            {debouncedSearchTerm && (
                                <button
                                    onClick={handleClearSearch}
                ...
                <div className="border-t border-gray-100 px-6 py-3 flex items-center justify-between">
                    <span className="text-xs text-gray-400">
                        {items.length > 0 ? `${items.length} stock type${items.length !== 1 ? 's' : ''} shown` : ''}
                    </span>
