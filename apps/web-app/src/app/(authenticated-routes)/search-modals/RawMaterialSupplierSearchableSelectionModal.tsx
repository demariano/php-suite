...
import { RawMaterialSupplierApi, RawMaterialSupplierDto } from '@data-access/index';
import { useEffect, useState } from 'react';

interface RawMaterialSupplierSearchableSelectionModalProps {
  show: boolean;
  title: string;
  selectedValue: string | null;
  onSelect: (supplier: RawMaterialSupplierDto) => void;
  onClose: () => void;
}
...
export default function RawMaterialSupplierSearchableSelectionModal({
  show,
  title,
  selectedValue,
  onSelect,
  onClose
}: RawMaterialSupplierSearchableSelectionModalProps) {
  ...
  const loadItems = async (searchQuery?: string) => {
    ...
    try {
      ...
      const itemsList = response.data.map((item: any) => ({
        id: item.rawMaterialSupplierId,
        name: item.rawMaterialSupplierName
      }));
      ...
      console.error('Error loading raw material suppliers:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSelect = (item: Item) => {
    // Create a full RawMaterialSupplierDto object
    const supplier: RawMaterialSupplierDto = {
      rawMaterialSupplierId: item.id,
      rawMaterialSupplierName: item.name,
        ...
        <div style={{ marginBottom: '20px' }}>
          <div style={{ display: 'flex', gap: '8px' }}>
            <input
              ...
              placeholder="Search raw material suppliers by name..."
              style={{
                flex: 1,
                padding: '12px 16px',
        ...
        <div style={{
          ...
          {loading ? (
            ...
            <div style={{
              ...
              {searchTerm ? `No raw material suppliers found matching "${searchTerm}"` : 'No raw material suppliers available'}
            </div>
          ) : (
            <div>
