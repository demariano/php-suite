...
import { ContractApi, ContractDto } from '@data-access/index';
import { useCallback, useEffect, useMemo, useRef, useState } from 'react';

interface ContractMultiSearchableSelectionModalProps {
    show: boolean;
    title: string;
    selectedValues?: string[];
    onSelectMultiple?: (contracts: ContractDto[]) => void;
    onClose?: () => void;
}
...
export default function ContractMultiSearchableSelectionModal({
    show,
    title,
    selectedValues,
    onSelectMultiple,
    onClose,
}: ContractMultiSearchableSelectionModalProps) {
    ...
    useEffect(() => {
        ...
        const hydrateSelected = async () => {
            ...
            const results = await Promise.allSettled(ids.map((id) => ContractApi.getContractById(id)));
            if (cancelled) return;

            const hydrated: ContractDto[] = results.map((res, idx) => {
                const fallbackId = ids[idx];
                if (res.status !== 'fulfilled') {
                    return { contractId: fallbackId, contractName: fallbackId };
                }

                const value = res.value as unknown;
                const maybeAxios = value as { data?: ContractDto };
                const contract = maybeAxios?.data ?? (value as ContractDto);
                const contractId = contract?.contractId || fallbackId;
                const contractNo = contract?.contractNo;
                const contractName = contract?.contractName || contractNo || contractId;
                return { ...contract, contractId, contractName };
            });
        ...
        async (query?: string) => {
            ...
            try {
                ...
                if (effectiveQuery) {
                    ...
                    response = await ContractApi.getContractsByStatus(limit, 'ACTIVE', direction, cursor);
                }

                const list: Item[] = Array.isArray(response?.data)
                    ? response.data.map((c: ContractDto) => {
                          const id = c.contractId || '';
                          const contractNo = c.contractNo;
                          const contractName = c.contractName;
                          const displayName = contractNo
                              ? `${contractNo}${contractName ? ` - ${contractName}` : ''}`
                              : contractName || id;
                          return {
                              id,
                              name: displayName,
    ...
    const selectedIds = useMemo(() => new Set(selectedItems.map((c) => c.contractId)), [selectedItems]);

    const handleToggle = (item: Item) => {
        const contract: ContractDto = item.fullData ?? { contractId: item.id, contractName: item.name };
        const exists = selectedItems.some((c) => c.contractId === contract.contractId);
        if (exists) {
            setSelectedItems(selectedItems.filter((c) => c.contractId !== contract.contractId));
        } else {
            setSelectedItems([...selectedItems, contract]);
        }
    };
            ...
            <div className="bg-white rounded-2xl w-[520px] max-w-[90vw] max-h-[80vh] overflow-hidden shadow-2xl flex flex-col border border-gray-100">
                ...
                <div className="px-6 pb-3">
                    <p className="text-xs text-gray-500">
                        {selectedItems.length === 0
                            ? 'No contracts selected (All Contracts)'
                            : `${selectedItems.length} contract${selectedItems.length === 1 ? '' : 's'} selected`}
                    </p>
                </div>
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                            ...
                            <span className="text-sm font-medium">
                                {debouncedSearchTerm
                                    ? `No contracts matching "${debouncedSearchTerm}"`
                ...
                <div className="border-t border-gray-100 px-6 py-3 flex items-center justify-between">
                    <span className="text-xs text-gray-400">
                        {items.length > 0 ? `${items.length} contract${items.length !== 1 ? 's' : ''} shown` : ''}
                    </span>
