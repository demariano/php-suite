...
import { SalesTypeApi, SalesTypeDto } from '@data-access/index';
import { useCallback, useEffect, useMemo, useRef, useState } from 'react';

interface SalesTypeMultiSearchableSelectionModalProps {
    show: boolean;
    title: string;
    selectedValues?: string[];
    onSelectMultiple?: (salesTypes: SalesTypeDto[]) => void;
    onClose?: () => void;
}
...
export default function SalesTypeMultiSearchableSelectionModal({
    show,
    title,
    selectedValues,
    onSelectMultiple,
    onClose,
}: SalesTypeMultiSearchableSelectionModalProps) {
    ...
    const [selectedItems, setSelectedItems] = useState<SalesTypeDto[]>([]);
    ...
    useEffect(() => {
        ...
        const hydrateSelected = async () => {
            ...
            const results = await Promise.allSettled(ids.map((id) => SalesTypeApi.getSalesTypeById(id)));
            if (cancelled) return;

            const hydrated: SalesTypeDto[] = results.map((res, idx) => {
                const fallbackId = ids[idx];
                if (res.status !== 'fulfilled') {
                    return { salesTypeId: fallbackId, salesTypeName: fallbackId };
                }

                const value = res.value as unknown;
                const maybeAxios = value as { data?: SalesTypeDto };
                const salesType = maybeAxios?.data ?? (value as SalesTypeDto);
                const salesTypeId = salesType?.salesTypeId || fallbackId;
                const salesTypeName = salesType?.salesTypeName || salesTypeId;
                return { ...salesType, salesTypeId, salesTypeName };
            });
    ...
    const loadItems = useCallback(
        async (searchQuery?: string) => {
            ...
            try {
                ...
                let response: SalesTypesResponse;
                if (effectiveQuery) {
                    response = await SalesTypeApi.getSalesTypesByName(effectiveQuery, limit, direction, cursor);
                } else {
                    response = await SalesTypeApi.getSalesTypesByStatus(limit, 'ACTIVE', direction, cursor);
                }

                const list: Item[] = Array.isArray(response?.data)
                    ? response.data.map((s: SalesTypeDto) => ({
                          id: s.salesTypeId || '',
                          name: s.salesTypeName || s.salesTypeId || '',
                          fullData: s,
                      }))
                    : [];
    ...
    const selectedIds = useMemo(() => new Set(selectedItems.map((s) => s.salesTypeId)), [selectedItems]);

    const handleToggle = (item: Item) => {
        const salesType: SalesTypeDto = item.fullData ?? { salesTypeId: item.id, salesTypeName: item.name };
        const exists = selectedItems.some((s) => s.salesTypeId === salesType.salesTypeId);
        if (exists) {
            setSelectedItems(selectedItems.filter((s) => s.salesTypeId !== salesType.salesTypeId));
        } else {
            setSelectedItems([...selectedItems, salesType]);
        }
    };
            ...
            <div className="bg-white rounded-2xl w-[520px] max-w-[90vw] max-h-[80vh] overflow-hidden shadow-2xl flex flex-col border border-gray-100">
                ...
                <div className="px-6 pb-3">
                    <p className="text-xs text-gray-500">
                        {selectedItems.length === 0
                            ? 'No sales types selected (All Sales Types)'
                            : `${selectedItems.length} sales type${selectedItems.length === 1 ? '' : 's'} selected`}
                    </p>
                </div>
                ...
                <div className="flex-1 overflow-y-auto px-3 py-2">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                            ...
                            <span className="text-sm">Loading sales types...</span>
                        </div>
                    ) : items.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                            <svg
                            ...
                            <span className="text-sm font-medium">
                                {debouncedSearchTerm
                                    ? `No sales types matching "${debouncedSearchTerm}"`
                                    : 'No sales types available'}
                            </span>
                            {debouncedSearchTerm && (
                                <button
                                    onClick={handleClearSearch}
                ...
                <div className="border-t border-gray-100 px-6 py-3 flex items-center justify-between">
                    <span className="text-xs text-gray-400">
                        {items.length > 0 ? `${items.length} sales type${items.length !== 1 ? 's' : ''} shown` : ''}
                    </span>
