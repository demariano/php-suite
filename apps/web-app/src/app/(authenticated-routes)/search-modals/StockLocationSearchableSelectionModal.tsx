...
import { StockLocationApi, StockLocationDto } from '@data-access/index';
import { useEffect, useState } from 'react';

interface StockLocationSearchableSelectionModalProps {
    show: boolean;
    title: string;
    selectedValue: string | null;
    onSelect: (location: StockLocationDto) => void;
    onClose: () => void;
}
...
export default function StockLocationSearchableSelectionModal({
    show,
    title,
    selectedValue,
    onSelect,
    onClose,
}: StockLocationSearchableSelectionModalProps) {
    ...
    const loadItems = async (searchQuery?: string) => {
        ...
        try {
            ...
            if (searchQuery) {
                response = await (StockLocationApi as any).searchStockLocationsByName(
                    searchQuery,
                    limit,
                    direction,
                    cursor
                ...
                response = await StockLocationApi.getStockLocationsByStatus(limit, 'ACTIVE', direction, cursor);
            }

            const itemsList = response.data.map((item: any) => ({
                id: item.stockLocationId,
                name: item.stockLocationName,
            }));
    ...
    const handleSelect = (item: Item) => {
        // Create a full StockLocationDto object
        const location: StockLocationDto = {
            stockLocationId: item.id,
            stockLocationName: item.name,
                ...
                <div className="px-6 pb-4">
                    <div className="relative">
                        ...
                        <input
                            ...
                            placeholder="Search stock locations by name..."
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                            ...
                            <span className="text-sm font-medium">
                                {searchTerm ? `No stock locations matching "${searchTerm}"` : 'No stock locations available'}
                            </span>
                            {searchTerm && (
