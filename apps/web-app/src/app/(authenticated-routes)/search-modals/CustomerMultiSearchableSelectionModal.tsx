...
import { CustomerApi, CustomerDto, CustomersResponse } from '@data-access/index';
import { useCallback, useEffect, useMemo, useRef, useState } from 'react';

interface CustomerMultiSearchableSelectionModalProps {
    show: boolean;
    title: string;
    selectedValues?: string[];
    onSelectMultiple?: (customers: CustomerDto[]) => void;
    onClose?: () => void;
}
...
interface Item {
    id: string;
    name: string;
    fullData?: CustomerDto;
}

export default function CustomerMultiSearchableSelectionModal({
    show,
    title,
    selectedValues,
    onSelectMultiple,
    onClose,
}: CustomerMultiSearchableSelectionModalProps) {
    ...
    useEffect(() => {
        ...
        const hydrateSelected = async () => {
            ...
            const results = await Promise.allSettled(ids.map((id) => CustomerApi.getCustomerById(id)));
            if (cancelled) return;

            const hydrated: CustomerDto[] = results.map((res, idx) => {
                const fallbackId = ids[idx];
                if (res.status !== 'fulfilled') {
                    return { customerId: fallbackId, customerName: fallbackId };
                }

                const value = res.value as unknown;
                const maybeAxios = value as { data?: CustomerDto };
                const customer = maybeAxios?.data ?? (value as CustomerDto);
                const customerId = customer?.customerId || fallbackId;
                const customerName = customer?.customerName || customerId;
    ...
    const loadItems = useCallback(
        async (searchQuery?: string) => {
            ...
            try {
                ...
                if (effectiveQuery) {
                    response = await CustomerApi.getCustomersByName(effectiveQuery, limit, direction, cursor);
                } else {
                    response = await CustomerApi.getCustomersByStatus(limit, 'ACTIVE', direction, cursor);
                }

                const list: Item[] = Array.isArray(response?.data)
                    ? response.data.map((c: CustomerDto) => ({
                          id: c.customerId || '',
    ...
    const selectedIds = useMemo(() => new Set(selectedItems.map((c) => c.customerId)), [selectedItems]);

    const handleToggle = (item: Item) => {
        const customer: CustomerDto = item.fullData ?? { customerId: item.id, customerName: item.name };
        const exists = selectedItems.some((c) => c.customerId === customer.customerId);
        if (exists) {
            setSelectedItems(selectedItems.filter((c) => c.customerId !== customer.customerId));
        } else {
            ...
            <div className="bg-white rounded-2xl w-[520px] max-w-[90vw] max-h-[80vh] overflow-hidden shadow-2xl flex flex-col border border-gray-100">
                ...
                <div className="px-6 pb-3">
                    <p className="text-xs text-gray-500">
                        {selectedItems.length === 0
                            ? 'No customers selected (All Customers)'
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                            ...
                            <span className="text-sm font-medium">
                                {debouncedSearchTerm
                                    ? `No customers matching "${debouncedSearchTerm}"`
                ...
                <div className="border-t border-gray-100 px-6 py-3 flex items-center justify-between">
                    <span className="text-xs text-gray-400">
                        {items.length > 0 ? `${items.length} customer${items.length !== 1 ? 's' : ''} shown` : ''}
                    </span>
