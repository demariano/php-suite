...
interface CustomerTypeSearchableSelectionModalProps {
  show: boolean;
  title: string;
  selectedValue: string | null;
  onSelect: (customerType: CustomerTypeDto) => void;
  onClose: () => void;
}
...
export default function CustomerTypeSearchableSelectionModal({
  show,
  title,
  selectedValue,
  onSelect,
  onClose
}: CustomerTypeSearchableSelectionModalProps) {
  ...
  const loadItems = async (searchQuery?: string) => {
    ...
    try {
      ...
      response = await CustomerTypeApi.getCustomerTypesByStatus(
        limit,
        'ACTIVE',
        direction,
        cursor,
  ...
  const handleSelect = (item: Item) => {
    // Create a full CustomerTypeDto object
    const customerType: CustomerTypeDto = {
            ...
            <div className="bg-white rounded-2xl w-[520px] max-w-[90vw] max-h-[80vh] overflow-hidden shadow-2xl flex flex-col border border-gray-100">
                ...
                <div className="flex-1 overflow-y-auto px-3 py-2">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                            ...
                            <span className="text-sm">Loading customer types...</span>
                        </div>
                    ) : items.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                            ...
                            <span className="text-sm font-medium">
                                {searchTerm ? `No types matching "${searchTerm}"` : 'No types available'}
                            </span>
                            {searchTerm && (
                ...
                <div className="border-t border-gray-100 px-6 py-3 flex items-center justify-between">
                    <span className="text-xs text-gray-400">
                        {items.length > 0 ? `${items.length} ${items.length !== 1 ? 'customer types' : 'customer type'} shown` : ''}
                    </span>
