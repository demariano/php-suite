...
import { ProductPriceTypeApi, ProductPriceTypeDto } from '@data-access/index';
import { useEffect, useState } from 'react';

interface ProductPriceTypeSearchableSelectionModalProps {
  show: boolean;
  title: string;
  selectedValue: string | null;
  onSelect: (priceType: ProductPriceTypeDto) => void;
  onClose: () => void;
}
...
export default function ProductPriceTypeSearchableSelectionModal({
  show,
  title,
  selectedValue,
  onSelect,
  onClose
}: ProductPriceTypeSearchableSelectionModalProps) {
  ...
  const loadItems = async (searchQuery?: string) => {
    ...
    try {
      ...
      const itemsList = response.data.map((item: any) => ({
        id: item.productPriceTypeId,
        name: item.productPriceTypeName
      }));
  ...
  const handleSelect = (item: Item) => {
    // Create a full ProductPriceTypeDto object
    const priceType: ProductPriceTypeDto = {
      productPriceTypeId: item.id,
      productPriceTypeName: item.name,
            ...
            <div className="bg-white rounded-2xl w-[520px] max-w-[90vw] max-h-[80vh] overflow-hidden shadow-2xl flex flex-col border border-gray-100">
                ...
                <div className="px-6 pb-4">
                    <div className="relative">
                        ...
                        <input
                            ...
                            placeholder="Search product price types by name..."
                ...
                <div className="flex-1 overflow-y-auto px-3 py-2">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                            ...
                            <span className="text-sm">Loading price types...</span>
                        </div>
                    ) : items.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                            ...
                            <span className="text-sm font-medium">
                                {searchTerm ? `No product price types matching "${searchTerm}"` : 'No product price types available'}
                            </span>
                            {searchTerm && (
                ...
                <div className="border-t border-gray-100 px-6 py-3 flex items-center justify-between">
                    <span className="text-xs text-gray-400">
                        {items.length > 0 ? `${items.length} ${items.length !== 1 ? 'price types' : 'price type'} shown` : ''}
                    </span>
