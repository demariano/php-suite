...
import { ProductDealApi, ProductDealDto } from '@data-access/index';
import { useCallback, useEffect, useMemo, useRef, useState } from 'react';

interface ProductDealMultiSearchableSelectionModalProps {
    show: boolean;
    title: string;
    selectedValues?: string[];
    onSelectMultiple?: (deals: ProductDealDto[]) => void;
    onClose?: () => void;
}
...
export default function ProductDealMultiSearchableSelectionModal({
    show,
    title,
    selectedValues,
    onSelectMultiple,
    onClose,
}: ProductDealMultiSearchableSelectionModalProps) {
    ...
    const [selectedItems, setSelectedItems] = useState<ProductDealDto[]>([]);
    ...
    useEffect(() => {
        ...
        const hydrateSelected = async () => {
            ...
            const results = await Promise.allSettled(ids.map((id) => ProductDealApi.getProductDealById(id)));
            if (cancelled) return;

            const hydrated: ProductDealDto[] = results.map((res, idx) => {
                const fallbackId = ids[idx];
                if (res.status !== 'fulfilled') {
                    return { productDealId: fallbackId, productDealName: fallbackId };
                }

                const value = res.value as unknown;
                const maybeAxios = value as { data?: ProductDealDto };
                const deal = maybeAxios?.data ?? (value as ProductDealDto);
                const productDealId = deal?.productDealId || fallbackId;
                const productDealName = deal?.productDealName || productDealId;
                return { ...deal, productDealId, productDealName };
            });
    ...
    const loadItems = useCallback(
        async (searchQuery?: string) => {
            ...
            try {
                ...
                if (effectiveQuery) {
                    response = await ProductDealApi.getProductDealsByName(effectiveQuery, limit, direction, cursor);
                } else {
                    response = await ProductDealApi.getProductDealsByStatus(limit, 'ACTIVE', direction, cursor);
                }

                const list: Item[] = Array.isArray(response?.data)
                    ? response.data.map((d: ProductDealDto) => ({
                          id: d.productDealId || '',
                          name: d.productDealName || d.productDealId || '',
                          fullData: d,
                      }))
                    : [];
    ...
    const selectedIds = useMemo(() => new Set(selectedItems.map((d) => d.productDealId)), [selectedItems]);

    const handleToggle = (item: Item) => {
        const deal: ProductDealDto = item.fullData ?? {
            productDealId: item.id,
            productDealName: item.name,
        };
        const exists = selectedItems.some((d) => d.productDealId === deal.productDealId);
        if (exists) {
            setSelectedItems(selectedItems.filter((d) => d.productDealId !== deal.productDealId));
        } else {
            ...
            <div className="bg-white rounded-2xl w-[520px] max-w-[90vw] max-h-[80vh] overflow-hidden shadow-2xl flex flex-col border border-gray-100">
                ...
                <div className="px-6 pb-4">
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-10 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-400 transition-all"
                            autoFocus
                        />
                        {searchTerm && (
                            <button
                ...
                <div className="px-6 pb-3">
                    <p className="text-xs text-gray-500">
                        {selectedItems.length === 0
                            ? 'No product deals selected (All Product Deals)'
                            : `${selectedItems.length} deal${selectedItems.length === 1 ? '' : 's'} selected`}
                    </p>
                </div>
                ...
                <div className="flex-1 overflow-y-auto px-3 py-2">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                            <svg
                                className="animate-spin h-6 w-6 mb-3 text-amber-500"
                            ...
                            <span className="text-sm">Loading product deals...</span>
                        </div>
                    ) : items.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                            <svg
                            ...
                            <span className="text-sm font-medium">
                                {debouncedSearchTerm
                                    ? `No deals matching "${debouncedSearchTerm}"`
                ...
                <div className="border-t border-gray-100 px-6 py-3 flex items-center justify-between">
                    <span className="text-xs text-gray-400">
                        {items.length > 0 ? `${items.length} deal${items.length !== 1 ? 's' : ''} shown` : ''}
                    </span>
                    <div className="flex items-center gap-2">
                        ...
                        <button
                            ...
                            className="inline-flex items-center px-3.5 py-1.5 text-xs font-medium rounded-md transition-all bg-amber-600 text-white hover:bg-amber-700"
                        >
