...
import { extractErrorMessage, StatusEnum, RawMaterialsStockApi, RawMaterialsStockDto, useEnv, useLocalStore, useSessionStore } from '@data-access/index';
...
export default function EditRawMaterialsStockPage({ params }: EditRawMaterialsStockPageProps) {
  ...
  useEffect(() => {
    const fetchStock = async () => {
      try {
        ...
        if ((stock.status === StatusEnum.FOR_APPROVAL || stock.status === StatusEnum.NEW_RECORD || stock.status === StatusEnum.FOR_DELETION) && isAdminUser) {
        ...
        const errorMessage = extractErrorMessage(err, 'Failed to load raw materials stock details. Please try again.');
  ...
  const handleSave = async (stock: RawMaterialsStockDto) => {
    try {
      ...
      const updatedStock = await RawMaterialsStockApi.updateRawMaterialsStock(params.id, {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      const approvedStock = await RawMaterialsStockApi.approveRawMaterialsStock(selectedStock.rawMaterialsStockId!, userRole);
  ...
  const handleDenyConfirm = async (approverMessage: string) => {
    ...
    try {
      ...
      const deniedStock = await RawMaterialsStockApi.denyRawMaterialsStock(selectedStock.rawMaterialsStockId!, approverMessage, userRole);
          ...
          <div className="rounded-xl border-2 border-red-300 bg-red-50 p-6 shadow-sm sm:p-8">
            ...
            {selectedStock.changeReason && (
              <div className="mt-6 rounded-lg border-2 border-red-200 bg-white p-4">
                ...
                <p className="mt-2 whitespace-pre-wrap font-mono text-sm text-gray-600 leading-relaxed">{selectedStock.changeReason}</p>
              </div>
            )}
          </div>
      ...
      <div className="space-y-6 animate-fadeIn rounded-xl border-2 border-blue-200 bg-white p-4 shadow-sm sm:p-6">
        ...
        <div className="space-y-4">
          <div className="border-2 border-gray-200 rounded-xl p-4 sm:p-6">
            ...
            {renderReadOnlyField('Quantity', approvalData.qty, 'bg-blue-500', 'qty')}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="border-2 border-gray-200 rounded-xl p-4 sm:p-6">
            ...
            {renderReadOnlyField('Raw Material', approvalData.rawMaterialName, 'bg-blue-500', 'rawMaterialName')}
            {renderReadOnlyField('Supplier', approvalData.rawMaterialSupplierName, 'bg-blue-500', 'rawMaterialSupplierName')}
            {renderReadOnlyField('Location', approvalData.rawMaterialsLocationName, 'bg-blue-500', 'rawMaterialsLocationName')}
            </div>
          </div>
        </div>
        ...
        {/* Action Buttons */}
        <div className="mt-8 flex flex-col gap-3 border-t-2 border-gray-200 pt-6 sm:flex-row sm:items-center sm:justify-between">
          {isAdminUser && (selectedStock?.status === StatusEnum.FOR_APPROVAL || selectedStock?.status === StatusEnum.NEW_RECORD || selectedStock?.status === StatusEnum.FOR_DELETION) ? (
            <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
              <button
                type="button"
                onClick={handleDeny}
    ...
    <div className="p-4 sm:p-6 space-y-6">
      ...
      <div>
        <nav className="flex items-center gap-2">
          ...
          <span className="text-gray-400">/</span>
          <a href="/inventory/raw-materials-stock" className="text-blue-500 no-underline text-sm hover:text-blue-600 transition-colors duration-200">
            Raw Materials Stock
          </a>
          <span className="text-gray-400">/</span>
              ...
              <div className="flex gap-2 flex-nowrap">
                <button
                  ...
                  className={`flex-shrink-0 px-5 py-3 rounded-lg font-semibold text-sm transition-colors ${
                    getTabColorClasses(selectedStock.status || StatusEnum.ACTIVE, activeTab === 'details')
                  }`}
                >
