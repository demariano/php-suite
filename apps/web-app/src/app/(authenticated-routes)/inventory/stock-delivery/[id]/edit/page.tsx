...
export default function EditStockDeliveryPage({ params }: EditStockDeliveryPageProps) {
    ...
    useEffect(() => {
        const fetchStockDelivery = async () => {
            try {
                ...
                if (
                    (stockDelivery.status === StatusEnum.FOR_APPROVAL ||
                        stockDelivery.status === StatusEnum.NEW_RECORD ||
                        stockDelivery.status === StatusEnum.FOR_DELETION) &&
                    isAdminUser
                ) {
                ...
                const errorMessage = extractErrorMessage(
                    err,
                    'Failed to load stock delivery details. Please try again.'
                );
    ...
    const handleSave = async (stockDelivery: StockDeliveryDto) => {
        try {
            ...
            const updatedStockDelivery = await StockDeliveryApi.updateStockDelivery(
                params.id,
                {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            // Navigate back to stock delivery list immediately - notification will persist
            router.push('/inventory/stock-delivery');
        } catch (error) {
    ...
    const handleApprove = async () => {
        ...
        try {
            ...
            const approvedStockDelivery = await StockDeliveryApi.approveStockDelivery(
                selectedStockDelivery.stockDeliveryId!,
                userRole
            );
    ...
    const handleDenyConfirm = async (approverMessage: string) => {
        ...
        try {
            ...
            const deniedStockDelivery = await StockDeliveryApi.denyStockDelivery(
                selectedStockDelivery.stockDeliveryId!,
                approverMessage,
                userRole
            );
    ...
    const renderApprovalTab = () => {
        ...
        const hasArrayChanges = (fieldName: string): boolean => {
            ...
            const newValue = (selectedStockDelivery.forApprovalVersion as unknown as Record<string, unknown>)[
                fieldName
            ];
            ...
            <div className="space-y-6 animate-fadeIn rounded-xl border-2 border-green-400 bg-white p-4 shadow-sm sm:p-6">
                ...
                <div className="space-y-4">
                    <div className="rounded-xl border-2 border-gray-200 p-4 sm:p-6">
                        <div className="mb-4 flex items-center gap-3">
                            ...
                            <h3 className="text-base font-bold text-blue-600">Stock Delivery Information</h3>
                        </div>
                        <div className="grid grid-cols-1 gap-6">
                            {renderReadOnlyField('Document Number', approvalData.docno, 'bg-blue-500', 'docno')}
                            {renderReadOnlyField(
                                'Date Received',
                                approvalData.dateReceived,
                                'bg-blue-500',
                                'dateReceived'
                            )}
                            {renderReadOnlyField(
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-amber-700 mt-1">
                                                    {originalItems.length} record{originalItems.length !== 1 ? 's' : ''}{' '}
                ...
                <div className="mt-8 flex flex-col gap-3 border-t-2 border-gray-200 pt-6 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    [StatusEnum.FOR_APPROVAL, StatusEnum.NEW_RECORD, StatusEnum.FOR_DELETION].includes(
                        selectedStockDelivery.status as StatusEnum
                    ) ? (
                        <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
                            <button
                                type="button"
                                    ...
                                    <span className="flex items-center gap-2">
                                        ...
                                        {selectedStockDelivery && (
                                            <>
                                                ...
                                                <span>
                                                    {getStatusText(selectedStockDelivery.status || StatusEnum.ACTIVE)}
                                                </span>
                                            </>
                                        )}
                                    </span>
