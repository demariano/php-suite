...
export default function CreateRawMaterialUnitPage() {
  ...
  const [formData, setFormData] = useState<RawMaterialUnitDto>({
    rawMaterialUnitId: '',
    rawMaterialUnitName: '',
    changeReason: '',
    activityLogs: [],
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const errors: string[] = [];
    if (!formData.rawMaterialUnitName?.trim()) {
    ...
    try {
      ...
      setFlashNotification({
        title: 'Success!',
        message: 'Raw material unit created successfully!',
        alertType: 'success'
      });
      router.replace('/inventory/raw-material-units');
    } catch (err: any) {
      console.error('Failed to create raw material unit:', err);
      const message = err?.message || 'Failed to create raw material unit. Please try again.';
      setError(message);
      setFlashNotification({
        title: 'Error!',
        message,
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      ...
      <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Create New Raw Material Unit</h1>
                  ...
                  <div className="grid grid-cols-1 gap-6">
                    <div className="group">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData((prev) => ({ ...prev, rawMaterialUnitName: e.target.value }))
                        }
            ...
            <div className="mt-4 flex flex-col gap-3 border-t-2 border-gray-200 pt-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
                ...
                >
                  ...
                  {isSubmitting ? 'Submitting...' : 'Create Raw Material Unit'}
                </button>
                <button
                  type="button"
