...
export default function RawMaterialUnitsPage() {
    const { env } = useEnv();
    const { authedUser } = useLocalStore();
    const { setFlashNotification } = useSessionStore();
    const userRole = env.BYPASS_AUTH === 'ENABLED' ? authedUser?.userRole : undefined;

    const [items, setItems] = useState<RawMaterialUnitDto[]>([]);
    ...
    const fetchRecords = async (direction?: 'next' | 'prev', cursor?: string, customSize?: number) => {
        try {
            ...
            if (searchQuery.trim()) {
                ...
                response = await RawMaterialUnitApi.getRawMaterialUnits(size, direction, serializedCursor, userRole);
            }
    ...
    const tableData = items.map((item) => {
        ...
        return {
            supplierId: item.rawMaterialUnitId,
            supplierName: item.rawMaterialUnitName,
    ...
    const handleRowClick = (record: { supplierId?: string }) => {
        ...
        window.location.href = `/inventory/raw-material-units/${record.supplierId}/edit`;
    };
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex w-full items-center gap-3 sm:flex-1 sm:max-w-md">
                    <div className="flex-1">
                        <Input
                            placeholder="Filter raw material units"
                            value={searchQuery}
                ...
                <button
                    type="button"
                    onClick={() => (window.location.href = '/inventory/raw-material-units/create')}
                ...
                >
                    <Add size={18} />
                    New raw material unit
                </button>
            </div>

            <div className="hidden sm:block bg-white border border-gray-200 rounded-xl overflow-hidden shadow-lg">
                {isLoading ? (
                    <div className="p-10 text-center text-base text-gray-500">Loading raw material units...</div>
                ) : (
