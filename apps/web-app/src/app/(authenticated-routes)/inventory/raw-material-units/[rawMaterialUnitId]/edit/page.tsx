...
export default function EditRawMaterialUnitPage({ params }: { params: { rawMaterialUnitId: string } }) {
  ...
  useEffect(() => {
    const fetchUnit = async () => {
      if (!params.rawMaterialUnitId) return;
      ...
      try {
        ...
        if (unit && ([StatusEnum.FOR_APPROVAL, StatusEnum.NEW_RECORD, StatusEnum.FOR_DELETION] as StatusEnum[]).includes(unit.status || StatusEnum.ACTIVE) && isAdminUser) {
          setActiveTab('approval');
        } else {
          setActiveTab('details');
        }
      } catch (err: any) {
        console.error('Failed to fetch raw material unit:', err);
        const message = err?.message || 'Failed to load raw material unit details. Please try again.';
        setError(message);
    ...
    fetchUnit();
  }, [params.rawMaterialUnitId, env.BYPASS_AUTH, authedUser?.userRole, isAdminUser, setFlashNotification]);

  const handleSave = async () => {
    if (!selectedUnit?.rawMaterialUnitId) return;
    ...
    try {
      ...
      const updated = await RawMaterialUnitApi.updateRawMaterialUnit(
        selectedUnit.rawMaterialUnitId,
        updatedPayload,
        userRole
      );
      ...
      setFlashNotification({
        title: 'Success!',
        message: 'Raw material unit updated successfully!',
        alertType: 'success'
      });
      router.replace('/inventory/raw-material-units');
    } catch (err: any) {
      console.error('Failed to save raw material unit:', err);
      const message = err?.message || 'Failed to save raw material unit. Please try again.';
      setError(message);
      setFlashNotification({
        title: 'Error!',
        message,
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      const message = extractErrorMessage(err, 'Failed to approve raw material unit. Please try again.');
      setFlashNotification({
        title: 'Error',
        message,
        alertType: 'error'
  ...
  const handleDenyConfirm = async (reason: string) => {
    ...
    try {
      ...
      const message = extractErrorMessage(err, 'Failed to deny raw material unit changes. Please try again.');
      setFlashNotification({
        title: 'Error',
        message,
        alertType: 'error'
  ...
  const renderApprovalTab = () => {
    ...
    if (selectedUnit.status === StatusEnum.FOR_DELETION) {
      return (
          ...
          <div className="rounded-xl border-2 border-red-300 bg-red-50 p-6 shadow-sm sm:p-8">
            ...
            {selectedUnit.changeReason && (
              <div className="mt-6 rounded-lg border-2 border-red-200 bg-white p-4">
                ...
                <p className="mt-2 whitespace-pre-wrap font-mono text-sm text-gray-600 leading-relaxed">{selectedUnit.changeReason}</p>
              </div>
            )}
          </div>
    ...
    if (!selectedUnit.forApprovalVersion) return null;

    const isFieldChanged = createFieldChangeDetector(
      selectedUnit as Record<string, unknown>,
      selectedUnit.forApprovalVersion as Record<string, unknown>
    );
        ...
        <div className="space-y-4">
          <div className="rounded-xl border-2 border-gray-200 p-4 sm:p-6">
            <div className="mb-4 flex items-center gap-3">
              ...
              <h3 className="text-base font-bold text-blue-600">Raw Material Unit Information</h3>
            </div>
            <div className="grid grid-cols-1 gap-6">
              {renderReadOnlyField('Name', selectedUnit.forApprovalVersion.rawMaterialUnitName, 'bg-blue-500', isFieldChanged('rawMaterialUnitName'))}
            </div>
          </div>
        </div>

        <div className="mt-8 flex flex-col gap-3 border-t-2 border-gray-200 pt-6 sm:flex-row sm:items-center sm:justify-between">
          {isAdminUser && ([StatusEnum.FOR_APPROVAL, StatusEnum.NEW_RECORD, StatusEnum.FOR_DELETION] as StatusEnum[]).includes(selectedUnit.status || StatusEnum.ACTIVE) ? (
            <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
              <button
                type="button"
                onClick={() => setShowDenyDialog(true)}
  ...
  const renderLogsTab = () => (
    <div className="space-y-6 animate-fadeIn">
      <div className="rounded-xl border-2 border-gray-200 bg-white p-4 shadow-sm sm:p-6">
        ...
        {renderActivityLogsTable(selectedUnit?.activityLogs)}
      </div>
  ...
  if (isLoading && !selectedUnit) {
    return (
      <div className="flex min-h-96 items-center justify-center">
        <div className="text-gray-600">Loading raw material unit details...</div>
      </div>
    );
  }
    ...
    return (
      <div className="p-4 sm:p-6 space-y-6">
        <div className="bg-red-50 border border-red-200 text-red-600 px-4 py-3 rounded-lg flex justify-between items-center shadow-sm">
          <span>Raw material unit not found</span>
        </div>
      </div>
    );
  }
    ...
    <div className="p-4 sm:p-6 space-y-6">
      ...
      <div>
        <nav className="flex items-center gap-2">
          ...
          <span className="text-gray-400">/</span>
          <a href="/inventory/raw-material-units" className="text-blue-500 no-underline text-sm hover:text-blue-600 transition-colors duration-200">
            Raw Material Units
          </a>
          <span className="text-gray-400">/</span>
              ...
              <div className="flex flex-nowrap gap-2">
                <button
                  ...
                  className={`flex-shrink-0 rounded-lg px-5 py-3 text-sm font-semibold transition-colors ${
                    getTabColorClasses(selectedUnit.status || StatusEnum.ACTIVE, activeTab === 'details')
                  }`}
                >
                  <span className="flex items-center gap-2">
                    ...
                    <span>{getStatusText(selectedUnit.status || StatusEnum.ACTIVE)}</span>
                  </span>
                </button>
                ...
                <div className="space-y-6">
                  ...
                  {!isAdminUser && (
                    <ChangeReasonField
                      ...
                      onChange={(e) =>
                        setSelectedUnit((prev) => prev ? { ...prev, changeReason: e.target.value } : prev)
                      }
                        ...
                        <div className="grid grid-cols-1 gap-6">
                          <div className="group">
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setSelectedUnit((prev) => prev ? { ...prev, rawMaterialUnitName: e.target.value } : prev)
                              }
        ...
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-lg space-y-6">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-lg font-semibold text-gray-800 m-0">Deny Raw Material Unit Changes</h3>
            </div>
