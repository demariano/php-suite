...
export default function StockForm({
    ...
    const [formData, setFormData] = useState({
        ...
        totalQuantity: selectedStock?.totalQuantity?.toString() || '0',
    ...
    const totalQuantityFormatting = useWholeNumberFormatting(formData.totalQuantity);

    const currentStatus = selectedStock?.status ?? StatusEnum.ACTIVE;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const pendingVersion = (selectedStock?.forApprovalVersion || {}) as Record<string, any>;
    const isFieldChangedFn = createFieldChangeDetector(selectedStock, selectedStock?.forApprovalVersion);
