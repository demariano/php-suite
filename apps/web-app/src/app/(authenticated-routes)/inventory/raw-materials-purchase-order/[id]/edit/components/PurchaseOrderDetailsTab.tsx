...
export function PurchaseOrderDetailsTab({
    ...
    const fetchSuppliers = async () => {
        try {
            const response = await RawMaterialSupplierApi.getRawMaterialSuppliersByStatus(100, 'ACTIVE');
            setSuppliers(
                (response.data || []).map((s: any) => ({
                    _id: s.rawMaterialSupplierId || s._id,
                    supplierName: s.rawMaterialSupplierName || s.supplierName,
    ...
    const fetchRawMaterials = async () => {
        try {
            const response = await RawMaterialApi.getRawMaterialsByStatus(100, 'ACTIVE');
            setRawMaterials(
                (response.data || []).map((m: any) => ({
                    _id: m.rawMaterialId || m._id,
                    materialName: m.rawMaterialName || m.materialName,
                    unit: m.rawMaterialUnitName || m.unit || '',
    ...
    const handleRawMaterialSelect = (rawMaterial: RawMaterialDto) => {
        ...
        const rawMaterialAny = rawMaterial as any;
        const defaultUnit: RawMaterialUnitDto = {
            rawMaterialUnitId: rawMaterialAny.rawMaterialUnitId || '',
            rawMaterialUnitName: rawMaterialAny.rawMaterialUnitName || '',
        };
    ...
    const handleAddOrderItem = () => {
        ...
        const isDuplicate = formData.purchaseOrderDetails.some((item) => item.rawMaterialId === selectedRawMaterial);
        ...
        const newItem: OrderItem = {
            ...
            rawMaterialName: selectedRawMaterialObj?.rawMaterialName || (rawMaterial as any).materialName,
