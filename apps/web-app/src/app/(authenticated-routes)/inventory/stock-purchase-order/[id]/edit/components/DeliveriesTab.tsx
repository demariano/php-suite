...
import { useSessionStore } from '@data-access/index';
...
export function DeliveriesTab({
  ...
  // Calculate delivery statistics for each stock item
  const getDeliveryStats = (productId: string) => {
    const orderedItem = purchaseOrderDetails.find((item) => item.productId === productId);
    if (!orderedItem) return null;

    const alreadyDelivered = deliveredPurchaseOrderDetails.reduce((total, delivery) => {
      // Sum all matching stock items in this delivery (there could be multiple with different lot numbers)
      const matchingItems = delivery.stockItems.filter((si) => si.productId === productId);
    ...
    const deliverNow = Number(deliveryItems[productId]?.qty || 0);
  ...
  const totalDelivered = deliveredPurchaseOrderDetails.reduce((total, delivery) => {
    return total + delivery.stockItems.reduce((sum, si) => sum + si.deliveredQty, 0);
  }, 0);
  const completionPercentage = totalOrdered > 0 ? (totalDelivered / totalOrdered) * 100 : 0;

  const handleDeliveryItemChange = (productId: string, field: 'qty' | 'lotNo', value: string) => {
    setDeliveryItems((prev) => ({
      ...prev,
      [productId]: {
        ...prev[productId],
        qty: field === 'qty' ? value : prev[productId]?.qty || '',
        lotNo: field === 'lotNo' ? value : prev[productId]?.lotNo || '',
      },
    }));
  ...
  const validateDelivery = (): boolean => {
    ...
    purchaseOrderDetails.forEach((item) => {
      const stats = getDeliveryStats(item.productId);
      if (!stats) return;
      ...
      if (stats.deliverNow > 0) {
        ...
        if (qtyNum !== stats.deliverNow) {
          errors[item.productId] = 'Quantity must be a whole number (no decimals)';
          return;
        }

        // Check if exceeds ordered quantity
        if (stats.willExceed) {
          errors[item.productId] = `Cannot deliver more than ordered (${stats.orderedQty} ${item.productUnitName})`;
          return;
        }
      }
    });
  ...
  const handleSaveDelivery = () => {
    ...
    const stockItems: DeliveredItem[] = [];
    purchaseOrderDetails.forEach((item) => {
      const deliverNow = Number(deliveryItems[item.productId]?.qty || 0);
      if (deliverNow > 0) {
        stockItems.push({
          ...
          lotNo: deliveryItems[item.productId]?.lotNo || '',
        });
      }
    });
                ...
                <table className="w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {purchaseOrderDetails.map((item) => {
                      ...
                      return (
                        <tr key={item.productId} className={hasError ? 'bg-red-50' : 'hover:bg-gray-50'}>
                          ...
                          <td className="px-6 py-4">
                            <div className="space-y-1">
                              <input
                                type="number"
                                value={deliveryItems[item.productId]?.qty || ''}
                                onChange={(e) => handleDeliveryItemChange(item.productId, 'qty', e.target.value)}
                                disabled={isDisabled || isSubmitting}
                                step="1"
                                min="0"
                          ...
                          <td className="px-6 py-4">
                            <input
                              type="text"
                              value={deliveryItems[item.productId]?.lotNo || ''}
                              onChange={(e) => handleDeliveryItemChange(item.productId, 'lotNo', e.target.value)}
                              disabled={isDisabled || isSubmitting}
                      ...
                      <div className="flex items-center gap-4">
                        <div>
                          ...
                          <p className="text-xs text-gray-500 mt-1">
                            {delivery.stockItems.length} item(s) - {delivery.stockItems.reduce((sum, si) => sum + si.deliveredQty, 0)} total units
                          </p>
                        </div>
                      </div>
                    ...
                    <Disclosure.Panel className="border-t border-gray-200">
                      <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200">
                          ...
                          <tbody className="bg-white divide-y divide-gray-200">
                            {delivery.stockItems.map((si, siIndex) => (
                              <tr key={siIndex} className="hover:bg-gray-50 transition-colors">
                                <td className="px-6 py-4 text-sm font-medium text-gray-900">
                                  {si.productName}
                                </td>
                                <td className="px-6 py-4 text-sm text-gray-700">{si.productUnitName}</td>
                                <td className="px-6 py-4 text-sm font-semibold text-blue-600">
                                  {si.deliveredQty}
                                </td>
                                <td className="px-6 py-4 text-sm text-gray-700">{si.lotNo}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[1000]">
          <div className="bg-white rounded-xl p-6 w-full max-w-md mx-4 shadow-lg">
            ...
            <p className="text-sm text-gray-600 mb-6 leading-relaxed">
              ...
              {deliveryToDelete.stockLocationName && (
                <>
                  {' '}(Location: <strong>{deliveryToDelete.stockLocationName}</strong>)
                </>
              )}
              <br /><br />
              This will remove <strong>{deliveryToDelete.stockItems.length} item(s)</strong> from the delivery records and adjust stock quantities accordingly. This action cannot be undone.
            </p>
