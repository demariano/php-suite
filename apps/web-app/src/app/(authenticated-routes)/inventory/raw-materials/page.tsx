...
import { RawMaterialApi, RawMaterialDto, StatusEnum, useEnv, useLocalStore, useSessionStore } from '@data-access/index';
...
export default function RawMaterialsPage() {
    ...
    const fetchRecords = async (direction?: 'next' | 'prev', cursor?: string, customSize?: number) => {
        try {
            ...
            if (searchQuery.trim()) {
                ...
                response = await RawMaterialApi.getRawMaterials(size, direction, serializedCursor, userRole);
            }
    ...
    const handleRowClick = (record: { rawMaterialId?: string }) => {
        if (!record?.rawMaterialId) return;
        window.location.href = `/inventory/raw-materials/${record.rawMaterialId}/edit`;
    };
    ...
    return (
        <div className="p-4 sm:p-6 space-y-6">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <button
                    type="button"
                    onClick={() => (window.location.href = '/inventory/raw-materials/create')}
            ...
            <div className="hidden sm:block bg-white border border-gray-200 rounded-xl overflow-hidden shadow-lg">
                {isLoading ? (
                    <div className="p-10 text-center text-base text-gray-500">Loading raw materials...</div>
                ) : (
                                                ...
                                                {row.latestActivity ? (
                                                    <span
                                                        className={`px-2 py-1 rounded ${
                                                            row.latestActivity.style.bgColor ?? ''
                                                        } ${row.latestActivity.style.textColor ?? ''}`.trim()}
                                                    >
