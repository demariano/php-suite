...
export default function EditRawMaterialsLocationPage({ params }: { params: { rawMaterialsLocationId: string } }) {
  ...
  const [selectedLocation, setSelectedLocation] = useState<RawMaterialsLocationDto | null>(null);
  ...
  useEffect(() => {
    const fetchLocation = async () => {
      if (!params.rawMaterialsLocationId) return;

      try {
        setIsLoading(true);
        setError(null);
        const location = await RawMaterialsLocationApi.getRawMaterialsLocationById(
          params.rawMaterialsLocationId,
          userRole
        );
        setSelectedLocation(location);

        if (location && ([StatusEnum.FOR_APPROVAL, StatusEnum.NEW_RECORD, StatusEnum.FOR_DELETION] as StatusEnum[]).includes(location.status || StatusEnum.ACTIVE) && isAdminUser) {
          setActiveTab('approval');
        } else {
          setActiveTab('details');
        }
      } catch (err: any) {
        console.error('Failed to fetch raw materials location:', err);
        const message = err?.message || 'Failed to load raw materials location details. Please try again.';
        setError(message);
    ...
    fetchLocation();
  }, [params.rawMaterialsLocationId, env.BYPASS_AUTH, authedUser?.userRole, isAdminUser, setFlashNotification]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updated = await RawMaterialsLocationApi.updateRawMaterialsLocation(
        selectedLocation.rawMaterialsLocationId,
        updatedPayload,
        userRole
      );
      ...
      setFlashNotification({
        title: 'Success!',
        message: 'Raw materials location updated successfully!',
        alertType: 'success'
      });
      router.replace('/inventory/raw-materials-locations');
    } catch (err: any) {
      console.error('Failed to save raw materials location:', err);
      const message = err?.message || 'Failed to save raw materials location. Please try again.';
      setError(message);
      setFlashNotification({
        title: 'Error!',
        message,
  ...
  const handleApprove = async () => {
    ...
    try {
      setIsSaving(true);
      await RawMaterialsLocationApi.approveRawMaterialsLocation(selectedLocation.rawMaterialsLocationId, userRole);
      setFlashNotification({
        title: 'Success!',
        message: 'Raw materials location approved successfully!',
        alertType: 'success'
      });
      router.replace('/inventory/raw-materials-locations');
    } catch (err) {
      const message = extractErrorMessage(err, 'Failed to approve raw materials location. Please try again.');
      setFlashNotification({
        title: 'Error',
        message,
        alertType: 'error'
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      const message = extractErrorMessage(err, 'Failed to delete raw materials location. Please try again.');
      setFlashNotification({
        title: 'Error',
        message,
        alertType: 'error'
  ...
  const handleDenyConfirm = async (reason: string) => {
    ...
    try {
      setIsSaving(true);
      await RawMaterialsLocationApi.denyRawMaterialsLocation(
        selectedLocation.rawMaterialsLocationId,
        reason,
        userRole
      );
      setFlashNotification({
        title: 'Success!',
        message: 'Raw materials location changes denied successfully!',
        alertType: 'success'
      });
      router.replace('/inventory/raw-materials-locations');
    } catch (err) {
      const message = extractErrorMessage(err, 'Failed to deny raw materials location changes. Please try again.');
      setFlashNotification({
        title: 'Error',
        message,
        alertType: 'error'
  ...
  const renderApprovalTab = () => {
    ...
    if (selectedLocation.status === StatusEnum.FOR_DELETION) {
      return (
          ...
          <div className="rounded-xl border-2 border-red-300 bg-red-50 p-6 shadow-sm sm:p-8">
            ...
            {selectedLocation.changeReason && (
              <div className="mt-6 rounded-lg border-2 border-red-200 bg-white p-4">
                ...
                <p className="mt-2 whitespace-pre-wrap font-mono text-sm text-gray-600 leading-relaxed">{selectedLocation.changeReason}</p>
              </div>
            )}
          </div>
    ...
    if (!selectedLocation.forApprovalVersion) return null;

    const isFieldChanged = createFieldChangeDetector(
      selectedLocation as Record<string, unknown>,
      selectedLocation.forApprovalVersion as Record<string, unknown>
    );
        ...
        <div className="space-y-4">
          <div className="rounded-xl border-2 border-gray-200 p-4 sm:p-6">
            <div className="mb-4 flex items-center gap-3">
              ...
              <h3 className="text-base font-bold text-blue-600">Raw Materials Location Information</h3>
            </div>
            <div className="grid grid-cols-1 gap-6">
              {renderReadOnlyField('Name', selectedLocation.forApprovalVersion.rawMaterialsLocationName, 'bg-blue-500', isFieldChanged('rawMaterialsLocationName'))}
            </div>
          </div>
        </div>

        <div className="mt-8 flex flex-col gap-3 border-t-2 border-gray-200 pt-6 sm:flex-row sm:items-center sm:justify-between">
          {isAdminUser && ([StatusEnum.FOR_APPROVAL, StatusEnum.NEW_RECORD, StatusEnum.FOR_DELETION] as StatusEnum[]).includes(selectedLocation.status || StatusEnum.ACTIVE) ? (
            <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
              <button
                type="button"
                onClick={() => setShowDenyDialog(true)}
  ...
  if (isLoading && !selectedLocation) {
    return (
      <div className="flex min-h-96 items-center justify-center">
        <div className="text-gray-600">Loading raw materials location details...</div>
      </div>
    );
  }
    ...
    return (
      <div className="p-4 sm:p-6 space-y-6">
        <div className="bg-red-50 border border-red-200 text-red-600 px-4 py-3 rounded-lg flex justify-between items-center shadow-sm">
          <span>Raw materials location not found</span>
        </div>
      </div>
    );
  }
    ...
    <div className="p-4 sm:p-6 space-y-6">
      ...
      <div>
        <nav className="flex items-center gap-2">
          ...
          <span className="text-gray-400">/</span>
          <a href="/inventory/raw-materials-locations" className="text-blue-500 no-underline text-sm hover:text-blue-600 transition-colors duration-200">
              ...
              <div className="flex flex-nowrap gap-2">
                <button
                  ...
                  className={`flex-shrink-0 rounded-lg px-5 py-3 text-sm font-semibold transition-colors ${
                    getTabColorClasses(selectedLocation.status || StatusEnum.ACTIVE, activeTab === 'details')
                  }`}
                >
                  <span className="flex items-center gap-2">
                    ...
                    <span>{getStatusText(selectedLocation.status || StatusEnum.ACTIVE)}</span>
                  </span>
                </button>
                ...
                <div className="space-y-6">
                  ...
                  {!isAdminUser && (
                    <ChangeReasonField
                      ...
                      onChange={(e) =>
                        setSelectedLocation((prev) => prev ? { ...prev, changeReason: e.target.value } : prev)
                      }
                      ...
                      <div className="rounded-xl border-2 border-gray-200 p-4 sm:p-6">
                        ...
                        <div className="grid grid-cols-1 gap-6">
                          <div className="group">
                            ...
                            <input
                              type="text"
                              value={selectedLocation.rawMaterialsLocationName || ''}
                              onChange={(e) =>
                                setSelectedLocation((prev) => prev ? { ...prev, rawMaterialsLocationName: e.target.value } : prev)
                              }
        ...
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-lg space-y-6">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-lg font-semibold text-gray-800 m-0">Deny Raw Materials Location Changes</h3>
            </div>
