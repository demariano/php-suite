...
export default function RawMaterialsLocationsPage() {
    const { env } = useEnv();
    const { authedUser } = useLocalStore();
    const { setFlashNotification } = useSessionStore();
    const userRole = env.BYPASS_AUTH === 'ENABLED' ? authedUser?.userRole : undefined;

    const [items, setItems] = useState<RawMaterialsLocationDto[]>([]);
    ...
    const statusBadge = (status?: StatusEnum) => {
        ...
        const colorMap: Record<StatusEnum, string> = {
            ...
            [StatusEnum.FOR_DEACTIVATION]: 'bg-orange-500 text-white',
            [StatusEnum.INACTIVE]: 'bg-gray-400 text-white',
        ...
        return (
            <span
                className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold shadow-sm ${colorMap[effectiveStatus]}`}
            >
                {effectiveStatus
                    .replace(/_/g, ' ')
                    .toLowerCase()
    ...
    const tableData = items.map((item) => {
        ...
        return {
            supplierId: item.rawMaterialsLocationId,
            supplierName: item.rawMaterialsLocationName,
    ...
    const handleRowClick = (record: { supplierId?: string }) => {
        ...
        window.location.href = `/inventory/raw-materials-locations/${record.supplierId}/edit`;
    };
        ...
        <div className="p-4 sm:p-6 space-y-6">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex w-full items-center gap-3 sm:flex-1 sm:max-w-md">
                    <div className="flex-1">
                        <Input
                            placeholder="Filter raw materials locations"
                            value={searchQuery}
                ...
                <button
                    type="button"
                    onClick={() => (window.location.href = '/inventory/raw-materials-locations/create')}
                ...
                >
                    <Add size={18} />
                    New raw materials location
                </button>
            </div>

            <div className="hidden sm:block bg-white border border-gray-200 rounded-xl overflow-hidden shadow-lg">
                {isLoading ? (
                    <div className="p-10 text-center text-base text-gray-500">Loading raw materials locations...</div>
                ) : (
                ...
                <div className="space-y-4 sm:hidden">
                    {tableData.length > 0 ? (
                        ...
                        <div className="rounded-xl border border-dashed border-gray-300 bg-white p-6 text-center text-sm text-gray-500">
                            {searchQuery ? `No records found matching "${searchQuery}"` : 'No records found'}
                        </div>
                    )}
                </div>
            )}
            ...
            {isLoading && (
                <div className="rounded-xl border border-gray-200 bg-white p-6 text-center text-gray-500 shadow-sm sm:hidden">
                    Loading raw materials locations...
                </div>
            )}
