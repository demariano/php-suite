...
export default function EditVoucherPage({ params }: EditVoucherPageProps) {
  ...
  useEffect(() => {
    const fetchVoucher = async () => {
      try {
        ...
        if ((voucher.status === StatusEnum.FOR_APPROVAL || voucher.status === StatusEnum.NEW_RECORD || voucher.status === StatusEnum.FOR_DELETION) && isAdminUser) {
  ...
  const handleSave = async (voucher: VoucherDto) => {
    try {
      ...
      const updatedVoucher = await VoucherApi.updateVoucher(params.id, {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await VoucherApi.deleteVoucher(selectedVoucher.voucherId, selectedVoucher, userRole);
