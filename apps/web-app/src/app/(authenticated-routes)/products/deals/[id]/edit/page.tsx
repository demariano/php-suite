...
import { ProductApi, ProductDealDto, StatusEnum, useEnv, useLocalStore, useSessionStore } from '@data-access/index';
...
export default function EditDealPage({ params }: EditDealPageProps) {
  ...
  useEffect(() => {
    const fetchDeal = async () => {
      ...
      try {
        ...
        const deal = await ProductApi.getProductDealById(params.id, userRole);
        setSelectedDeal(deal);

        if ((deal.status === StatusEnum.FOR_APPROVAL || deal.status === StatusEnum.NEW_RECORD || deal.status === StatusEnum.FOR_DELETION) && isAdminUser) {
          setActiveTab('approval');
        } else {
          setActiveTab('details');
        }
      } catch (err: any) {
        console.error('Failed to fetch product deal:', err);
        setError(err.message || 'Failed to load product deal details. Please try again.');
        setFlashNotification({
          title: 'Error!',
          message: err.message || 'Failed to load product deal details. Please try again.',
          alertType: 'error'
        });
      } finally {
        setIsLoading(false);
  ...
  const handleSave = async (deal: ProductDealDto) => {
    ...
    try {
      ...
      const updatedRecord = await ProductApi.updateProductDeal(deal.productDealId, {
        ...deal,
        status: deal.status
      }, userRole);
      ...
      router.replace('/products/deals');
    } catch (err: any) {
      console.error('Failed to save product deal:', err);
      setError(err.message || 'Failed to save product deal. Please try again.');
      setFlashNotification({
        title: 'Error!',
        message: err.message || 'Failed to save product deal. Please try again.',
        alertType: 'error'
      });
    } finally {
      setIsLoading(false);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      console.error('Failed to delete product deal:', err);
      setError(err.message || 'Failed to delete product deal. Please try again.');
      setFlashNotification({
        title: 'Error!',
        message: err.message || 'Failed to delete product deal. Please try again.',
        alertType: 'error'
      });
    } finally {
      setIsLoading(false);
  ...
  const handleApproveRecord = async () => {
    ...
    try {
      ...
      console.error('Failed to approve product deal:', err);
      setError(err.message || 'Failed to approve product deal. Please try again.');
      setFlashNotification({
        title: 'Error!',
        message: err.message || 'Failed to approve product deal. Please try again.',
        alertType: 'error'
      });
    } finally {
      setIsLoading(false);
  ...
  const handleDenyRecord = async () => {
    ...
    try {
      ...
      setFlashNotification({
        title: 'Success!',
        message: 'Product Deal denied successfully!',
        alertType: 'success'
      });
      router.replace('/products/deals');
    } catch (err: any) {
      console.error('Failed to deny product deal:', err);
      setError(err.message || 'Failed to deny product deal. Please try again.');
      setFlashNotification({
        title: 'Error!',
        message: err.message || 'Failed to deny product deal. Please try again.',
        alertType: 'error'
      });
    } finally {
      setIsLoading(false);
  ...
  if (error && !selectedDeal) {
    return (
      <div className="p-6 bg-gradient-to-br from-gray-50 to-white min-h-screen">
        ...
        <button onClick={() => router.replace('/products/deals')} className="mt-4 px-4 py-2 bg-blue-500 text-white rounded-md">
          Back to Deals
        </button>
      </div>
    );
  ...
  if (!selectedDeal && !isLoading) {
    return (
      <div className="p-6 text-center text-gray-600">
        Product Deal not found or could not be loaded.
        <button onClick={() => router.replace('/products/deals')} className="mt-4 px-4 py-2 bg-blue-500 text-white rounded-md">
          Back to Deals
        </button>
      </div>
  ...
  const approvalVersionData: ProductDealDto = {
    ...selectedDeal,
    ...selectedDeal?.forApprovalVersion
  };
    ...
    <div className="p-6 bg-gradient-to-br from-gray-50 to-white min-h-screen">
      <div className="mb-6">
        <nav className="flex items-center gap-2">
          ...
          <span className="text-gray-400">/</span>
          <a href="/products/deals" className="text-blue-500 no-underline text-sm hover:text-blue-600 transition-colors duration-200">
            Deals
          </a>
          <span className="text-gray-400">/</span>
      ...
      <h1 className="text-3xl font-bold text-gray-900 mb-6">Edit Product Deal: {selectedDeal?.productDealName}</h1>
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
        ...
        <div className="flex border-b-2 border-gray-200 mb-5 bg-gray-50 rounded-t-xl p-1">
          ...
          {selectedDeal && (selectedDeal.status === StatusEnum.FOR_APPROVAL || selectedDeal.status === StatusEnum.NEW_RECORD || selectedDeal.status === StatusEnum.FOR_DELETION) && (
            <button
        ...
        {/* Tab Content */}
        <div>
          {activeTab === 'details' && selectedDeal && (
            <DealForm
              isCreateMode={false}
          ...
          {activeTab === 'approval' && !isLoading && selectedDeal && (
            <div>
              <div className="mb-5">
                {(selectedDeal.status === StatusEnum.FOR_APPROVAL || selectedDeal.status === StatusEnum.NEW_RECORD || selectedDeal.status === StatusEnum.FOR_DELETION) && (
                  <div className="bg-yellow-50 border border-yellow-200 rounded-md p-3 mb-4 flex items-center gap-2">
                    ...
                    <span className="text-yellow-800 text-sm">
                      {selectedDeal.status === StatusEnum.FOR_DELETION ? 'This record is pending deletion approval.' : 'These are the proposed changes awaiting approval.'}
                    </span>
                  </div>
                )}
                  ...
                  <div className="bg-gray-50 border border-gray-200 rounded-xl p-5 mb-6 shadow-md">
                    ...
                    {Object.entries(approvalVersionData).map(([key, value]) => {
                      if (
                        ...
                        key === 'status' || key === 'productDealId'
                      ) {
                        return null;
                      }

                      const originalValue = (selectedDeal as any)[key];
              ...
              {isAdminUser && (selectedDeal.status === StatusEnum.FOR_APPROVAL || selectedDeal.status === StatusEnum.NEW_RECORD || selectedDeal.status === StatusEnum.FOR_DELETION) && (
          ...
          {activeTab === 'logs' && !isLoading && selectedDeal && (
            <div>
              <div className="mb-5">
                <h3 className="text-base font-semibold text-gray-800 mb-3">Recent Activity</h3>
                {selectedDeal?.activityLogs && selectedDeal.activityLogs.length > 0 ? (
                  <div className="bg-gray-50 p-4 rounded-md border border-gray-200 max-h-72 overflow-y-auto">
                    {selectedDeal.activityLogs.map((log, index) => (
                      <div
                        key={index}
                        className={`py-2 ${
                          index < selectedDeal.activityLogs!.length - 1 ? 'border-b border-gray-200' : ''
                        }`}
                      >
                        {log}
                      </div>
