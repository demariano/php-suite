...
export default function EditProductClassPage({ params }: EditProductClassPageProps) {
    ...
    useEffect(() => {
        const fetchProductClass = async () => {
            try {
                ...
                if (
                    (productClass.status === StatusEnum.FOR_APPROVAL ||
                        productClass.status === StatusEnum.NEW_RECORD ||
                        productClass.status === StatusEnum.FOR_DELETION) &&
                    isAdminUser
                ) {
                ...
                const errorMessage = extractErrorMessage(
                    err,
                    'Failed to load product class details. Please try again.'
                );
    ...
    const handleSave = async (productClass: ProductClassDto) => {
        try {
            ...
            const updatedProductClass = await ProductClassApi.updateProductClass(
                params.id,
                {
    ...
    const handleApprove = async () => {
        ...
        try {
            ...
            const approvedProductClass = await ProductClassApi.approveProductClass(
    ...
    const handleDenyConfirm = async (approverMessage: string) => {
        ...
        try {
            ...
            const deniedProductClass = await ProductClassApi.denyProductClass(
                selectedProductClass.productClassId,
                approverMessage,
                userRole
            );
    ...
    const renderApprovalTab = () => {
        ...
        const isFieldChanged = createFieldChangeDetector(
            selectedProductClass as Record<string, unknown>,
            selectedProductClass.forApprovalVersion as Record<string, unknown> | undefined
        );
            ...
            <div className="space-y-6 animate-fadeIn rounded-xl border-2 border-blue-200 bg-white p-4 shadow-sm sm:p-6">
                ...
                <div className="mt-8 flex flex-col gap-3 border-t-2 border-gray-200 pt-6 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    [StatusEnum.FOR_APPROVAL, StatusEnum.NEW_RECORD, StatusEnum.FOR_DELETION].includes(
                        selectedProductClass.status as StatusEnum
                    ) ? (
                        <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
                            <button
                                type="button"
                                    ...
                                    <span className="flex items-center gap-2">
                                        ...
                                        {selectedProductClass && (
                                            <>
                                                ...
                                                <span>
                                                    {getStatusText(selectedProductClass.status || StatusEnum.ACTIVE)}
                                                </span>
                                            </>
                                        )}
                                    </span>
