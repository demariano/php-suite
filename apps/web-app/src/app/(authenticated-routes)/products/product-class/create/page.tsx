...
import { ProductClassApi, ProductClassDto, extractErrorMessage, useEnv, useLocalStore, useSessionStore } from '@data-access/index';
...
export default function CreateProductClassPage() {
  ...
  const handleSave = async (productClass: ProductClassDto) => {
    try {
      ...
      await ProductClassApi.createProductClass({
      ...
      console.error('Error creating product class:', error);
      const errorMessage = extractErrorMessage(error, 'Failed to create product class. Please try again.');
      setFlashNotification({
        title: 'Error',
