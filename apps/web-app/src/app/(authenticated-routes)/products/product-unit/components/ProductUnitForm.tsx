  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div className="mt-8 flex flex-col gap-3 border-t-2 border-gray-200 pt-6 sm:flex-row sm:items-center sm:justify-between">
        {!isCreateMode && selectedProductUnit?.status === StatusEnum.ACTIVE ? (
        ...
        ) : !isCreateMode && isAdminUser && selectedProductUnit && 
          [StatusEnum.FOR_APPROVAL, StatusEnum.NEW_RECORD, StatusEnum.FOR_DELETION].includes(selectedProductUnit.status as StatusEnum) ? (
          <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
            ...
            >
              ...
              {isLoading ? 'Processing...' : selectedProductUnit.status === StatusEnum.FOR_DELETION ? 'Deny Deletion' : 'Deny Changes'}
            </button>
            <button
              type="button"
              onClick={onApprove}
            ...
            >
              ...
              {isLoading ? 'Processing...' : selectedProductUnit.status === StatusEnum.FOR_DELETION ? 'Approve Deletion' : 'Approve Changes'}
            </button>
          </div>
        ) : (
