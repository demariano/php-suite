...
import { ProductUnitApi, ProductUnitDto, extractErrorMessage, StatusEnum, useEnv, useLocalStore, useSessionStore } from '@data-access/index';
...
export default function EditProductUnitPage({ params }: EditProductUnitPageProps) {
  ...
  useEffect(() => {
    const fetchProductUnit = async () => {
      try {
        ...
        if ((productUnit.status === StatusEnum.FOR_APPROVAL || productUnit.status === StatusEnum.NEW_RECORD || productUnit.status === StatusEnum.FOR_DELETION) && isAdminUser) {
  ...
  const handleSave = async (productUnit: ProductUnitDto) => {
    try {
      ...
      const updatedProductUnit = await ProductUnitApi.updateProductUnit(params.id, {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      const approvedProductUnit = await ProductUnitApi.approveProductUnit(selectedProductUnit.productUnitId, userRole);
  ...
  const handleDeny = async () => {
    ...
    try {
      ...
      const deniedProductUnit = await ProductUnitApi.denyProductUnit(selectedProductUnit.productUnitId, userRole);
  ...
  const renderApprovalTab = () => {
    ...
    if (selectedProductUnit.status === StatusEnum.FOR_DELETION) {
      return (
          ...
          <div className="rounded-xl border-2 border-red-300 bg-red-50 p-6 shadow-sm sm:p-8">
            ...
            {selectedProductUnit.changeReason && (
              <div className="mt-6 rounded-lg border-2 border-red-200 bg-white p-4">
                ...
                <p className="mt-2 whitespace-pre-wrap font-mono text-sm text-gray-600 leading-relaxed">{selectedProductUnit.changeReason}</p>
              </div>
            )}
          </div>
    ...
    if (!selectedProductUnit.forApprovalVersion) return null;

    const approvalData = selectedProductUnit.forApprovalVersion;
    ...
    const isFieldChanged = (fieldName: string): boolean => {
      if (!selectedProductUnit?.forApprovalVersion) return false;

      const originalValue = (selectedProductUnit as unknown as Record<string, unknown>)[fieldName];
      const newValue = (selectedProductUnit.forApprovalVersion as unknown as Record<string, unknown>)[fieldName];

      if (!(fieldName in selectedProductUnit.forApprovalVersion)) return false;
        ...
        <div className="space-y-4">
          <div className="rounded-xl border-2 border-gray-200 p-4 sm:p-6">
            ...
            <div className="grid grid-cols-1 gap-6">
              {renderReadOnlyField('Product Unit Name', approvalData.productUnitName, 'bg-blue-500', 'productUnitName')}
            </div>
          </div>
        </div>

        <div className="mt-8 flex flex-col gap-3 border-t-2 border-gray-200 pt-6 sm:flex-row sm:items-center sm:justify-between">
          {isAdminUser && selectedProductUnit && ([StatusEnum.FOR_APPROVAL, StatusEnum.NEW_RECORD, StatusEnum.FOR_DELETION].includes(selectedProductUnit.status as StatusEnum)) ? (
            <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
              <button
                type="button"
                onClick={handleDeny}
    ...
    return (
      <div className="space-y-6 animate-fadeIn">
        <div className="rounded-xl border-2 border-gray-200 bg-white p-4 shadow-sm sm:p-6">
          ...
          {selectedProductUnit?.activityLogs && selectedProductUnit.activityLogs.length > 0 ? (
            <div className="max-h-80 overflow-y-auto rounded-lg border border-gray-200 bg-gray-50">
              <ul className="divide-y divide-gray-200 text-sm text-gray-700">
                {selectedProductUnit.activityLogs.map((log, index) => (
                  <li
                    key={index}
                    className="px-4 py-3"
                  >
    ...
    <div className="p-4 sm:p-6 space-y-6">
      ...
      <div>
        <nav className="flex items-center gap-2">
          ...
          <span className="text-gray-400">/</span>
          <a href="/products/product-unit" className="text-blue-500 no-underline text-sm hover:text-blue-600 transition-colors duration-200">
            Product Unit
          </a>
          <span className="text-gray-400">/</span>
              ...
              <div className="flex flex-nowrap gap-2">
                <button
                  ...
                  className={`flex-shrink-0 rounded-lg px-5 py-3 text-sm font-semibold transition-colors ${
                    getTabColorClasses(selectedProductUnit.status || StatusEnum.ACTIVE, activeTab === 'details')
                  }`}
                >
                  <span className="flex items-center gap-2">
                    ...
                    {selectedProductUnit && (
                      <>
                        ...
                        <span>{getStatusText(selectedProductUnit.status || StatusEnum.ACTIVE)}</span>
                      </>
                    )}
                  </span>
                </button>
