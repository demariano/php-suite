...
export default function EditProductPriceTypePage({ params }: EditProductPriceTypePageProps) {
  ...
  useEffect(() => {
    const fetchProductPriceType = async () => {
      try {
        setIsLoading(true);
        ...
        if ((productPriceType.status === StatusEnum.FOR_APPROVAL || productPriceType.status === StatusEnum.NEW_RECORD || productPriceType.status === StatusEnum.FOR_DELETION) && isAdminUser) {
        ...
        console.error('Error fetching product price type:', err);
        const errorMessage = extractErrorMessage(err, 'Failed to load product price type details. Please try again.');
  ...
  const handleSave = async (productPriceType: ProductPriceTypeDto) => {
    try {
      ...
      const updatedProductPriceType = await ProductPriceTypeApi.updateProductPriceType(params.id, {
        ...
        changeReason: productPriceType.changeReason
      }, userRole);
      
      setSelectedProductPriceType(updatedProductPriceType);
      setFlashNotification({
        title: 'Success!',
      ...
      console.error('Error updating product price type:', error);
      const errorMessage = extractErrorMessage(error, 'Failed to update product price type. Please try again.');
      setFlashNotification({
        title: 'Error',
        message: errorMessage,
        alertType: 'error'
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      console.error('Error deleting product price type:', error);
      const errorMessage = extractErrorMessage(error, 'Failed to delete product price type. Please try again.');
      setFlashNotification({
        title: 'Error',
        message: errorMessage,
        alertType: 'error'
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      const approvedProductPriceType = await ProductPriceTypeApi.approveProductPriceType(selectedProductPriceType.productPriceTypeId, userRole);
      setSelectedProductPriceType(approvedProductPriceType);
      setFlashNotification({
        title: 'Success!',
      ...
      console.error('Error approving product price type:', err);
      const errorMessage = extractErrorMessage(err, 'Failed to approve product price type. Please try again.');
      setFlashNotification({
        title: 'Error',
        message: errorMessage,
        alertType: 'error'
  ...
  const handleDeny = async () => {
    ...
    try {
      ...
      setFlashNotification({
        title: 'Success!',
        message: 'Product Price Type changes denied successfully!',
        alertType: 'success'
      });
      ...
      console.error('Error denying product price type:', err);
      const errorMessage = extractErrorMessage(err, 'Failed to deny product price type. Please try again.');
      setFlashNotification({
        title: 'Error',
        message: errorMessage,
        alertType: 'error'
  ...
  if (!selectedProductPriceType && !isLoading) {
    return (
      <div className="p-4 sm:p-6 space-y-6">
        <div className="bg-red-50 border border-red-200 text-red-600 px-4 py-3 rounded-lg flex justify-between items-center shadow-sm">
          <span>Product Price Type not found</span>
        </div>
      </div>
    );
  }
  ...
  const renderApprovalTab = () => {
    if (!selectedProductPriceType) return null;

    if (selectedProductPriceType.status === StatusEnum.FOR_DELETION) {
      return (
        <div className="space-y-6 animate-fadeIn">
          <div className="rounded-xl border-2 border-red-300 bg-red-50 p-6 shadow-sm sm:p-8">
            ...
            {selectedProductPriceType.changeReason && (
              <div className="mt-6 rounded-lg border-2 border-red-200 bg-white p-4">
                ...
                <p className="mt-2 whitespace-pre-wrap font-mono text-sm text-gray-600 leading-relaxed">{selectedProductPriceType.changeReason}</p>
              </div>
            )}
          </div>
    ...
    if (!selectedProductPriceType.forApprovalVersion) return null;
        ...
        <div className="space-y-4">
          <div className="rounded-xl border-2 border-gray-200 p-4 sm:p-6">
            ...
            <div className="grid grid-cols-1 gap-6">
              {renderReadOnlyField('Product Price Type Name', approvalData.productPriceTypeName, 'bg-blue-500', 'productPriceTypeName')}
            </div>
          </div>
        </div>

        <div className="mt-8 flex flex-col gap-3 border-t-2 border-gray-200 pt-6 sm:flex-row sm:items-center sm:justify-between">
          {isAdminUser && selectedProductPriceType && ([StatusEnum.FOR_APPROVAL, StatusEnum.NEW_RECORD, StatusEnum.FOR_DELETION].includes(selectedProductPriceType.status as StatusEnum)) ? (
            <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
              <button
                type="button"
                onClick={handleDeny}
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      ...
      {isLoading && !selectedProductPriceType && (
        <div className="flex min-h-96 items-center justify-center">
          <div className="text-gray-600">Loading product price type details...</div>
        </div>
      )}
              ...
              <div className="flex flex-nowrap gap-2">
                <button
                  ...
                  className={`flex-shrink-0 rounded-lg px-5 py-3 text-sm font-semibold transition-colors ${
                    getTabColorClasses(selectedProductPriceType.status || StatusEnum.ACTIVE, activeTab === 'details')
                  }`}
                >
                  <span className="flex items-center gap-2">
                    ...
                    {selectedProductPriceType && (
                      <>
                        ...
                        <span>{getStatusText(selectedProductPriceType.status || StatusEnum.ACTIVE)}</span>
                      </>
                    )}
                  </span>
                </button>

                {selectedProductPriceType.status !== StatusEnum.ACTIVE && (
                  <button
            ...
            <div className="bg-white p-4 sm:p-6">
              {activeTab === 'details' && (
                <ProductPriceTypeForm
                  isCreateMode={false}
                  selectedProductPriceType={selectedProductPriceType}
                  successMessage={null}
                  onSave={handleSave}
