...
export default function EditProductPriceTypePage({ params }: EditProductPriceTypePageProps) {
    ...
    useEffect(() => {
        const fetchProductPriceType = async () => {
            try {
                ...
                if (
                    (productPriceType.status === StatusEnum.FOR_APPROVAL ||
                        productPriceType.status === StatusEnum.NEW_RECORD ||
                        productPriceType.status === StatusEnum.FOR_DELETION) &&
                    isAdminUser
                ) {
                ...
                const errorMessage = extractErrorMessage(
                    err,
                    'Failed to load product price type details. Please try again.'
                );
    ...
    const handleSave = async (productPriceType: ProductPriceTypeDto) => {
        try {
            ...
            const updatedProductPriceType = await ProductPriceTypeApi.updateProductPriceType(
                params.id,
                {
    ...
    const handleApprove = async () => {
        ...
        try {
            ...
            const approvedProductPriceType = await ProductPriceTypeApi.approveProductPriceType(
                selectedProductPriceType.productPriceTypeId,
                userRole
            );
    ...
    const handleDenyConfirm = async (approverMessage: string) => {
        ...
        try {
            ...
            const deniedProductPriceType = await ProductPriceTypeApi.denyProductPriceType(
                selectedProductPriceType.productPriceTypeId,
                approverMessage,
                userRole
            );
            ...
            <div className="space-y-6 animate-fadeIn rounded-xl border-2 border-blue-200 bg-white p-4 shadow-sm sm:p-6">
                ...
                <div className="space-y-4">
                    <div className="rounded-xl border-2 border-gray-200 p-4 sm:p-6">
                        <div className="mb-4 flex items-center gap-3">
                            ...
                            <h3 className="text-base font-bold text-blue-600">Product Price Type Information</h3>
                        </div>
                        <div className="grid grid-cols-1 gap-6">
                            {renderReadOnlyField(
                                'Product Price Type Name',
                                approvalData.productPriceTypeName,
                                'bg-blue-500',
                ...
                <div className="mt-8 flex flex-col gap-3 border-t-2 border-gray-200 pt-6 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    [StatusEnum.FOR_APPROVAL, StatusEnum.NEW_RECORD, StatusEnum.FOR_DELETION].includes(
                        selectedProductPriceType.status as StatusEnum
                    ) ? (
                        <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
                            <button
                                type="button"
