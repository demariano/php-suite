...
interface ProductPriceTypeFormProps {
    ...
    selectedProductPriceType: ProductPriceTypeDto | null;
    successMessage: string | null;
    onSave: (productPriceType: ProductPriceTypeDto) => void;
    onDelete: () => void;
    onCancel: () => void;
    isAdminUser?: boolean;
    onApprove?: () => void;
...
export default function ProductPriceTypeForm({
    isCreateMode,
    ...
    const currentStatus = selectedProductPriceType?.status ?? StatusEnum.NEW_RECORD;
    const pendingVersion = useMemo(
        () => (selectedProductPriceType?.forApprovalVersion ?? {}) as any,
        [selectedProductPriceType?.forApprovalVersion]
    );
    ...
    useEffect(() => {
        if (!isCreateMode && selectedProductPriceType) {
            setFormData({
                productPriceTypeName: selectedProductPriceType.productPriceTypeName || '',
                changeReason: selectedProductPriceType.changeReason || '',
            });
        }
    }, [isCreateMode, selectedProductPriceType]);

    // Use shared field change detection utility
    const isFieldChanged = createFieldChangeDetector(
        (selectedProductPriceType ?? {}) as any,
        (selectedProductPriceType?.forApprovalVersion as any) ?? undefined
    );
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!formData.productPriceTypeName?.trim()) {
            errors.push('Product Price Type Name is required.');
        }
        ...
        if (isCreateMode) {
            const newProductPriceType = {
                productPriceTypeName: formData.productPriceTypeName,
                status: StatusEnum.NEW_RECORD,
            } as ProductPriceTypeDto;
            onSave(newProductPriceType);
        } else {
            // Determine status based on user role
            const newStatus = isAdminUser ? StatusEnum.ACTIVE : StatusEnum.FOR_APPROVAL;

            const updatedProductPriceType = {
                ...selectedProductPriceType,
                productPriceTypeName: formData.productPriceTypeName,
                status: newStatus,
                changeReason: formData.changeReason?.trim() || undefined,
            } as ProductPriceTypeDto;
            onSave(updatedProductPriceType);
        }
    };

    const isFormDisabled = !isCreateMode && selectedProductPriceType?.status !== StatusEnum.ACTIVE;
    ...
    return (
        <form onSubmit={handleSubmit}>
            ...
            {showApprovalUI && selectedProductPriceType?.changeReason && (
                <ChangeReasonReadOnly value={selectedProductPriceType.changeReason} />
            )}
                            ...
                            {showApprovalUI ? (
                                <>
                                    {renderFieldWithInlineDiff(
                                        ...
                                        'productPriceTypeName',
                                        selectedProductPriceType?.productPriceTypeName,
                                        pendingVersion.productPriceTypeName,
                                        'bg-blue-500'
                                    )}
                                </>
                            ) : (
                                <div className="group">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) =>
                                            setFormData((prev) => ({ ...prev, productPriceTypeName: e.target.value }))
                                        }
                                        placeholder={isCreateMode ? 'Enter product price type name' : ''}
                                        disabled={isFormDisabled}
                ...
                <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
                    ...
                    {(isCreateMode || currentStatus === StatusEnum.ACTIVE) && (
                        ...
                        >
                            ...
                            {isCreateMode ? 'Create Product Price Type' : 'Save Changes'}
                        </button>
                    )}
