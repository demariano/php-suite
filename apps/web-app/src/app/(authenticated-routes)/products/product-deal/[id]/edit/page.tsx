...
export default function EditProductDealPage({ params }: EditProductDealPageProps) {
    ...
    useEffect(() => {
        const fetchProductDeal = async () => {
            try {
                ...
                if (
                    (productDeal.status === StatusEnum.FOR_APPROVAL ||
                        productDeal.status === StatusEnum.NEW_RECORD ||
                        productDeal.status === StatusEnum.FOR_DELETION) &&
                    isAdminUser
                ) {
    ...
    const handleSave = async (productDeal: ProductDealDto) => {
        try {
            ...
            const updatedProductDeal = await ProductDealApi.updateProductDeal(
                params.id,
                {
    ...
    const handleApprove = async () => {
        ...
        try {
            ...
            const approvedProductDeal = await ProductDealApi.approveProductDeal(
                selectedProductDeal.productDealId,
                userRole
            );
    ...
    const handleDenyConfirm = async (approverMessage: string) => {
        ...
        try {
            ...
            const deniedProductDeal = await ProductDealApi.denyProductDeal(
                selectedProductDeal.productDealId,
                approverMessage,
                userRole
            );
    ...
    const renderApprovalTab = () => {
        ...
        const isFieldChanged = createFieldChangeDetector(
            selectedProductDeal as Record<string, unknown>,
            selectedProductDeal.forApprovalVersion as Record<string, unknown> | undefined
        );
            ...
            <div className="space-y-6 animate-fadeIn rounded-xl border-2 border-blue-200 bg-white p-4 shadow-sm sm:p-6">
                ...
                <div className="mt-8 flex flex-col gap-3 border-t-2 border-gray-200 pt-6 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    [StatusEnum.FOR_APPROVAL, StatusEnum.NEW_RECORD, StatusEnum.FOR_DELETION].includes(
                        selectedProductDeal.status as StatusEnum
                    ) ? (
                        <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
                            <button
                                type="button"
                                    ...
                                    <span className="flex items-center gap-2">
                                        ...
                                        {selectedProductDeal && (
                                            <>
                                                ...
                                                <span>
                                                    {getStatusText(selectedProductDeal.status || StatusEnum.ACTIVE)}
                                                </span>
                                            </>
                                        )}
                                    </span>
