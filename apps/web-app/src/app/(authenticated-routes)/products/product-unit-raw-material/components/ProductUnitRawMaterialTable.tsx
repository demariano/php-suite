...
interface ProductUnitRawMaterialTableProps {
    isLoading: boolean;
...
export default function ProductUnitRawMaterialTable({
    isLoading,
    tableData,
    headers,
    searchQuery,
    ...
    onPrevious,
    onNext,
}: ProductUnitRawMaterialTableProps) {
    return (
        <>
            {/* Desktop Table */}
            <div className="hidden sm:block bg-white border border-gray-200 rounded-xl overflow-hidden shadow-lg">
                {isLoading ? (
                    <div className="p-10 text-center text-gray-500 text-base">Loading product unit raw materials...</div>
                ) : (
                                            ...
                                            {headers.map((header) => (
                                                <td key={header.key} className="px-6 py-5 text-sm">
                                                    ...
                                                    {header.key === 'latestActivity' && (
                                                        item.latestActivity ? (
                                                            <span 
                                                                className={`px-2 py-1 rounded ${item.latestActivity.style.bgColor} ${item.latestActivity.style.textColor}`}
                                                                title={item.latestActivity.text}
                                                            >
                                                                {item.latestActivity.text.length > 50 
                                                                    ? `${item.latestActivity.text.substring(0, 50)}...` 
                            ...
                            >
                                ...
                                {item.latestActivity && (
                                    <div className="mt-2">
                                        ...
                                        <span 
                                            className={`px-2 py-1 rounded text-xs ${item.latestActivity.style.bgColor} ${item.latestActivity.style.textColor}`}
                                        >
                                            {item.latestActivity.text.length > 60 
                                                ? `${item.latestActivity.text.substring(0, 60)}...` 
                                                : item.latestActivity.text}
                                        </span>
                                    </div>
                                )}
