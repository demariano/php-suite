...
interface PriceTypeTableProps {
  ...
  searchTerm: string;
  onRowClick: (priceType: ProductPriceTypeDto) => void;
  pageSize: number;
...
export default function PriceTypeTable({
  isLoading,
  tableData,
  headers,
  searchTerm,
        ...
        <div className="overflow-x-auto" style={{ backgroundColor: 'white' }}>
          <table className="w-full border-collapse min-w-full" style={{ backgroundColor: 'white' }}>
            ...
            <tbody style={{ backgroundColor: 'white' }}>
              {tableData.length > 0 ? tableData.map((priceType, index) => (
                <tr 
                  key={priceType.productPriceTypeId}
                  onClick={() => onRowClick(priceType)}
