...
export default function EditTermsPage({ params }: EditTermsPageProps) {
    ...
    useEffect(() => {
        const fetchTerms = async () => {
            try {
                ...
                if (
                    (terms.status === StatusEnum.FOR_APPROVAL ||
                        terms.status === StatusEnum.NEW_RECORD ||
                        terms.status === StatusEnum.FOR_DELETION ||
                        terms.status === StatusEnum.FOR_DEACTIVATION) &&
                    isAdminUser
                ) {
    ...
    const renderApprovalTab = () => {
        ...
        if (selectedTerms.status === StatusEnum.FOR_DEACTIVATION) {
            return (
                <div className="space-y-6 animate-fadeIn">
            ...
            <div className="space-y-6 animate-fadeIn rounded-xl border-2 border-blue-200 bg-white p-4 shadow-sm sm:p-6">
                ...
                <div className="mt-8 flex flex-col gap-3 border-t-2 border-gray-200 pt-6 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    [StatusEnum.FOR_APPROVAL, StatusEnum.NEW_RECORD, StatusEnum.FOR_DELETION].includes(
                        selectedTerms.status as StatusEnum
                    ) ? (
                        <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
                            <button
                                type="button"
