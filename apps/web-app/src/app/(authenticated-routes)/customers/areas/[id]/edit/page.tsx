...
export default function EditAreaPage({ params }: EditAreaPageProps) {
    ...
    useEffect(() => {
        const fetchArea = async () => {
            try {
                ...
                if (
                    (area.status === StatusEnum.FOR_APPROVAL ||
                        area.status === StatusEnum.NEW_RECORD ||
                        area.status === StatusEnum.FOR_DEACTIVATION) &&
                    isAdminUser
                ) {
    ...
    const renderApprovalTab = () => {
        ...
        // If status is FOR_DEACTIVATION, show deactivation message (soft delete for Master Data)
        if (selectedArea.status === StatusEnum.FOR_DEACTIVATION) {
            return (
                <div className="space-y-6 animate-fadeIn">
                    <div className="bg-orange-50 border-2 border-orange-300 rounded-xl p-8 shadow-sm">
                        ...
                        {selectedArea.changeReason && (
                            <div className="mt-6 p-4 bg-white border-2 border-orange-200 rounded-lg">
                                <p className="text-sm font-semibold text-gray-700 mb-2">Deactivation Reason:</p>
        ...
        return (
            <div className="space-y-6 animate-fadeIn rounded-xl border-2 border-blue-200 bg-white p-4 shadow-sm sm:p-6">
                ...
                <div className="space-y-4">
                    <div className="border-2 border-gray-200 rounded-xl p-4 sm:p-6">
                        <div className="mb-4 flex items-center gap-3">
                            ...
                            <h3 className="text-base font-bold text-blue-600">Area Information</h3>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {renderReadOnlyField('Area Name', approvalData.areaName, 'areaName')}
                            {renderReadOnlyField(
                                'Territory Manager',
                                approvalData.territoryManagerName,
                ...
                <div className="mt-8 flex flex-col gap-3 border-t-2 border-gray-200 pt-6 sm:flex-row sm:items-center sm:justify-between">
                    {isAdminUser &&
                    (selectedArea?.status === StatusEnum.FOR_APPROVAL ||
                        selectedArea?.status === StatusEnum.NEW_RECORD ||
