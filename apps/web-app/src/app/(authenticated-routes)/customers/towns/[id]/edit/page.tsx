...
import { extractErrorMessage, StatusEnum, TownApi, TownDto, useEnv, useLocalStore, useSessionStore } from '@data-access/index';
...
export default function EditTownPage({ params }: EditTownPageProps) {
  ...
  const renderApprovalTab = () => {
    ...
    if (selectedTown.status === StatusEnum.FOR_DELETION) {
      return (
          ...
          <div className="rounded-xl border-2 border-red-300 bg-red-50 p-6 shadow-sm sm:p-8">
            ...
            {selectedTown.changeReason && (
              <div className="mt-6 rounded-lg border-2 border-red-200 bg-white p-4">
                ...
                <p className="mt-2 whitespace-pre-wrap font-mono text-sm text-gray-600 leading-relaxed">{selectedTown.changeReason}</p>
              </div>
            )}
          </div>
    ...
    if (!selectedTown.forApprovalVersion) return null;
    ...
    return (
      <div className="space-y-6 animate-fadeIn rounded-xl border-2 border-blue-200 bg-white p-4 shadow-sm sm:p-6">
        ...
        {/* Action Buttons */}
        <div className="mt-8 flex flex-col gap-3 border-t-2 border-gray-200 pt-6 sm:flex-row sm:items-center sm:justify-between">
          {isAdminUser && selectedTown && ([StatusEnum.FOR_APPROVAL, StatusEnum.NEW_RECORD, StatusEnum.FOR_DELETION].includes(selectedTown.status as StatusEnum)) ? (
            <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
              <button
                type="button"
                onClick={handleDeny}
              ...
              <div className="flex gap-2 flex-nowrap">
                <button
                  ...
                  className={`flex-shrink-0 px-5 py-3 rounded-lg font-semibold text-sm transition-colors ${
                    getTabColorClasses(selectedTown.status || StatusEnum.ACTIVE, activeTab === 'details')
                  }`}
                >
                  <span className="flex items-center gap-2">
                    ...
                    {selectedTown && (
                      <>
                        ...
                        <span>{getStatusText(selectedTown.status || StatusEnum.ACTIVE)}</span>
                      </>
                    )}
                  </span>
                </button>
