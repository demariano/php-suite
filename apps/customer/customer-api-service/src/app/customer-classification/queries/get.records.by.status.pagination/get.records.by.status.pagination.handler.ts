...
export class GetRecordsByStatusPaginationHandler implements IQueryHandler<GetRecordsByStatusPaginationQuery> {
    ...
    constructor(
        ...
        private readonly db: CustomerClassificationDatabaseServiceAbstract
    ) {}

    async execute(query: GetRecordsByStatusPaginationQuery): Promise<ResponseDto<PageDto<CustomerClassificationDto>>> {
        this.logger.log(`Processing pagination request for customer classifications with status: ${query.status}`);
        try {
            ...
            return new ResponseDto<PageDto<CustomerClassificationDto>>(records, HTTP_STATUS_OK);
        } catch (error) {
    ...
    private handleError(error: unknown): never {
        ...
        throw new BadRequestException('Failed to retrieve paginated customer classifications');
    }
}
