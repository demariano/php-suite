...
async function createSwaggerConfig() {
    return new DocumentBuilder()
        .setTitle('CUSTOMER-EVENT-HANDLER-SERVICE')
        .setDescription('CUSTOMER-EVENT-HANDLER-SERVICE API')
        .setVersion('1.0')
        .addBearerAuth(
            {
                type: 'http',
...
async function bootstrapServer() {
    ...
    if (process.env.LOCALSTACK_STATUS === 'ENABLED') {
        ...
        Logger.log(`🚀 CUSTOMER-EVENT-HANDLER-SERVICE is running on: http://localhost:${port}/api`);
        Logger.log(`🚀 CUSTOMER-EVENT-HANDLER-SERVICE Swagger Endpoint : http://localhost:${port}/api/swagger`);
    }
}
