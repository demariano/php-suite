...
@CommandHandler(ApproveRawMaterialsLocationCommand)
export class ApproveRawMaterialsLocationHandler implements ICommandHandler<ApproveRawMaterialsLocationCommand> {
    protected readonly logger = new Logger(ApproveRawMaterialsLocationHandler.name);
    ...
    async execute(
        command: ApproveRawMaterialsLocationCommand
    ): Promise<ResponseDto<RawMaterialsLocationDto | ErrorResponseDto>> {
        this.logger.log(`Processing approve request for raw materials location: ${command.recordId}`);
        ...
        try {
            ...
            this.logger.log(`Raw materials location approved successfully: ${command.recordId}`);
            return result;
        } catch (error) {
    ...
    private async approveRecord(
        ...
        })}, Raw materials location approved by ${user.username}, status set to ${StatusEnum.ACTIVE}`;
