...
import { DenyRawMaterialSupplierCommand } from './deny.command';

const HTTP_STATUS_OK = 200;
const ACTIVITY_LOGS_LIMIT = 10;

@CommandHandler(DenyRawMaterialSupplierCommand)
export class DenyRawMaterialSupplierHandler implements ICommandHandler<DenyRawMaterialSupplierCommand> {
    protected readonly logger = new Logger(DenyRawMaterialSupplierHandler.name);
    ...
    async execute(
        command: DenyRawMaterialSupplierCommand
    ): Promise<ResponseDto<RawMaterialSupplierDto | ErrorResponseDto>> {
        this.logger.log(`Processing deny request for raw material supplier: ${command.recordId}`);
        ...
        try {
            ...
            this.logger.log(`Raw material supplier denied successfully: ${command.recordId}`);
            return result;
        } catch (error) {
        ...
        switch (existingRecord.status) {
            ...
            default:
                throw new BadRequestException(
                    `Cannot deny raw material supplier with status: ${existingRecord.status}`
                );
        }
    }
    ...
    private async denyRecord(
        ...
        })}, Raw material supplier changes denied by ${user.username}`;
        ...
        if (existingRecord.approverMessage) {
            existingRecord.activityLogs.push(
                `Date: ${new Date().toLocaleString('en-US', {
                    timeZone: 'Asia/Manila',
                })}, Raw material supplier denied by ${user.username}, approver message: ${
    ...
    private async denyDeletion(existingRecord: RawMaterialSupplierDto): Promise<ResponseDto<RawMaterialSupplierDto>> {
        existingRecord.changeReason = null;
        existingRecord.status = StatusEnum.ACTIVE;

        const activityLog = `Date: ${new Date().toLocaleString('en-US', {
            timeZone: 'Asia/Manila',
        })}, Raw material supplier deletion denied`;
        existingRecord.activityLogs = [...(existingRecord.activityLogs || []), activityLog];
        existingRecord.activityLogs = reduceArrayContents(existingRecord.activityLogs, ACTIVITY_LOGS_LIMIT);
    ...
    private async deleteRecord(existingRecord: RawMaterialSupplierDto): Promise<ResponseDto<RawMaterialSupplierDto>> {
        existingRecord.changeReason = null;
