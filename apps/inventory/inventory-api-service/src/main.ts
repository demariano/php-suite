...
async function createSwaggerConfig() {
    return new DocumentBuilder()
        .setTitle('INVENTORY-API-SERVICE')
        .setDescription('INVENTORY-API-SERVICE API')
        .setVersion('1.0')
        .addBearerAuth(
            {
                type: 'http',
...
async function bootstrapServer() {
    ...
    Logger.log(`🚀 INVENTORY-API-SERVICE is running on: http://localhost:${port}/api`);
    Logger.log(`🚀 INVENTORY-API-SERVICE Swagger Endpoint : http://localhost:${port}/api/swagger`);
}
