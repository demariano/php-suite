...
async function createSwaggerConfig() {
    return new DocumentBuilder()
        .setTitle('LOCAL-TEST-SERVICE')
        .setDescription('LOCAL-TEST-SERVICE API')
        .setVersion('1.0')
        .addBearerAuth(
            {
                type: 'http',
...
async function bootstrapServer() {
    ...
    Logger.log(`🚀 LOCAL-TEST-SERVICE is running on: http://localhost:${port}/api`);
    Logger.log(`🚀 LOCAL-TEST-SERVICE Swagger Endpoint : http://localhost:${port}/api/swagger`);
}
