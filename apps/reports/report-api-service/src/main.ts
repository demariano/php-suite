...
async function createSwaggerConfig() {
    return new DocumentBuilder()
        .setTitle('REPORT-API-SERVICE')
        .setDescription('REPORT-API-SERVICE API')
        .setVersion('1.0')
        .addBearerAuth(
            {
                type: 'http',
...
async function bootstrapServer() {
    ...
    Logger.log(`🚀 REPORT-API-SERVICE is running on: http://localhost:${port}/api`);
    Logger.log(`🚀 REPORT-API-SERVICE Swagger Endpoint : http://localhost:${port}/api/swagger`);
}
