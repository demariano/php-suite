...
async function bootstrapServer() {
    ...
    if (process.env.LOCALSTACK_STATUS === 'ENABLED') {
        ...
        const port = process.env.PORT || <LAST_PORT_USED>;

        await app.listen(port);
        Logger.log(`🚀 [APP-NAME] is running on: http://localhost:${port}/api`);
        Logger.log(`🚀 [APP-NAME] Swagger Endpoint : http://localhost:${port}/api/swagger`);
    }
